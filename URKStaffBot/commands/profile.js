...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    fivemexports.urk.getConnected([parseInt(params[0])], function(connected) {
        userConnected = connected
    })
    fivemexports.ghmattimysql.execute("SELECT * FROM `urk_user_vehicles` WHERE user_id = ?", [params[0]], (cars) => {
        fivemexports.ghmattimysql.execute("SELECT * FROM `urk_user_data` WHERE user_id = ?", [params[0]], (userdata) => {
            fivemexports.ghmattimysql.execute("SELECT * FROM urk_warnings WHERE user_id = ?", [params[0]], (warnings) => {
                fivemexports.ghmattimysql.execute("SELECT * FROM urk_user_moneys WHERE user_id = ?", [params[0]], (money) => {
                    fivemexports.ghmattimysql.execute("SELECT * FROM urk_casino_chips WHERE user_id = ?", [params[0]], (chips) => {
                        fivemexports.ghmattimysql.execute("SELECT * FROM `urk_users` WHERE id = ?", [params[0]], (users) => {
                            fivemexports.ghmattimysql.execute("SELECT discord_id FROM `urk_verification` WHERE user_id = ?", [params[0]], (discordid) => {
