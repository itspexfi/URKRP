...
local c
local d
AddEventHandler("URK:onClientSpawn",function(e, f)
    if f then
        local g = function()
            if not c then
        ...
        local i = function(j)
            if not c and IsControlJustPressed(1, 51) then
                TriggerServerEvent("URK:requestStartCooking", j.recipe)
        ...
        for k, l in pairs(a) do
            for m, n in pairs(l) do
                tURK.createArea("cooking_" .. k .. "_" .. m,n.cookingPosition,1.5,6,g,h,i,{recipe = k, recipeTable = a[k], index = m})
                tURK.addMarker(n.cookingPosition.x,n.cookingPosition.y,n.cookingPosition.z - 0.2,0.5,0.5,0.5,0,50,255,170,50,20,false,false,true)
            end
        end
    end
end)
...
local function t()
    for p, q in pairs(b) do
        for r, s in pairs(q) do
            local u = tURK.loadModel(s.model)
...
AddEventHandler("URK:onClientSpawn",function(e, f)
    if f then
        ...
        for w, x in pairs(b) do
            tURK.createArea("cooking_staticobjects_" .. w, x[1].position, 100, 5, g, h, i, {})
        end
    end
end)
...
local function B()
    CreateThread(function()
        tURK.startCircularProgressBar("",3000,nil,function()end)
    end)
    local C = PlayerPedId()
    tURK.loadAnimDict("amb@prop_human_bum_bin@enter")
    TaskPlayAnim(C, "amb@prop_human_bum_bin@enter", "enter", 8.0, 8.0, -1, 1, 0.2)
    RemoveAnimDict("amb@prop_human_bum_bin@enter")
    Wait(1500)
    tURK.loadAnimDict("amb@prop_human_bum_bin@exit")
...
local function D(E)
    local F = true
    local G
    tURK.minigameCircularProgressBar({Difficulty = E, Timeout = 100000, onComplete = function(H)
        if H then
            G = true
        else
            G = false
...
function cookBBQ()
    ...
    tURK.startCircularProgressBar("",10000,nil,function()end)
    local L = 0
    ...
    while L < 3 do
        ...
        if H then
            ...
            tURK.announceMpSmallMsg("Disaster", "You dropped the burger patty, start again!", 6, 3000)
        end
        Wait(2500)
        tURK.startCircularProgressBar("",10000,nil,function()end)
        Wait(2500)
    ...
    tURK.announceMpSmallMsg("Success", "You made a burger!", 18, 3000)
    ClearPedTasks(C)
    StopParticleFxLooped(K, true)
end
local function M()
    local C = tURK.getPlayerPed()
    local N = tURK.getPlayerCoords()
    local u = tURK.loadModel("prop_food_bs_tray_02")
    ...
    SetModelAsNoLongerNeeded(u)
    tURK.loadAnimDict("anim@heists@box_carry@")
    TaskPlayAnim(C, "anim@heists@box_carry@", "idle", 2.0, 2.0, -1, 51, 0, false, false, false)
    Citizen.CreateThread(function()
        while tURK.doesAreaExist("temp_foodorder_complete") do
...
local function T(U)
    ...
    local W = tURK.addMarker(V.x, V.y, V.z + 0.3, 0.15, 0.15, 0.15, 255, 215, 0, 170, 50, 0, false, true, true)
    local X = tURK.addBlip(V.x, V.y, V.z, 1, 2, "Delivery Point", 0.6)
    SetBlipFlashes(X, true)
    tURK.createArea("temp_foodorder_complete",V,1.5,6.0,function()
        tURK.removeArea("temp_foodorder_complete")
        tURK.removeMarker(W)
        tURK.removeBlip(X)
...
RegisterNetEvent("URK:cookingInstruction",function(Z)
    for r, _ in pairs(R) do
        tURK.removeArea("temp_cooking_instruction")
        tURK.removeMarker(_)
    end
    for r, Y in pairs(S) do
        tURK.removeBlip(Y)
    ...
    local W = tURK.addMarker(V.x, V.y, V.z + 0.3, 0.15, 0.15, 0.15, 255, 215, 0, 170, 50, 0, false, true, true)
    table.insert(R, W)
    local X = tURK.addBlip(V.x, V.y, V.z, 1, 2, "Ingredient", 0.6)
    SetBlipFlashes(X, true)
    table.insert(S, X)
    local g = function()
    end
    ...
    local i = function()
        if IsControlJustPressed(1, 51) then
            ...
            if Z == "bbq" then
                ...
                tURK.removeMarker(W)
                T(function()
                    TriggerEvent("URK:playItemBoughtSound")
                    TriggerServerEvent("URK:pickupCookingIngredient", c, Z)
                    tURK.setCanAnim(true)
                    tURK.announceMpSmallMsg("Success", "You earned £4,000, now get back to work!", 18, 3000)
                end)
            else
                tURK.removeMarker(W)
                tURK.removeBlip(X)
    ...
    tURK.createArea("temp_cooking_instruction", V, 1.0, 6, g, h, i)
end)
RegisterNetEvent("URK:beginCooking",function(a0)
    TriggerEvent("URK:smallAnnouncement", "Cooking", "Make a " .. a0 .. ".", 15, 5000)
end)
RegisterNetEvent("URK:finishedCooking",function()
    c = nil
end)
