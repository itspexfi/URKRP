...
Citizen.CreateThread(function()
    while true do
        if m and tURK.getPlayerVehicle() ~= k.limitingVehicle then
            m = false
            SetVehicleMaxSpeed(tURK.getPlayerVehicle(), k.predictedMax)
            predictedMax = nil
            tURK.notify("~r~Vehicle Changed, stopping limiter")
        end
        Wait(500)
    end
end)
    ...
    Citizen.CreateThread(function()
        while k.limiter do
            ...
            if z ~= 0 then
                ...
                tURK.notify("~r~You are not in a vehicle!")
                k.limiter = false
            end
            Wait(100)
        end
...
RMenu.Add("vehicle_menu", "main", RageUI.CreateMenu("", "", tURK.getRageUIMenuWidth(), tURK.getRageUIMenuHeight(), "banners", "urk_settingsui"))
RMenu:Get("vehicle_menu", "main"):SetSubtitle("~r~VEHICLE OPTIONS")
    ...
    if RageUI.Visible(RMenu:Get("vehicle_menu", "main")) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.Button("Toggle Door Lock", "Select to toggle between doors locked and unlocked",{RightLabel=""},true,function(o,p,q)
                if q then
                    local L = tURK.getPlayerVehicle()
                    if L ~= 0 then
                        if GetVehicleDoorLockStatus(L) ~= 2 then
                            SetVehicleDoorsLocked(L, 2)
                            tURK.notify("~w~Doors are now ~r~Locked")
                        else
                            SetVehicleDoorsLocked(L, 0)
                            tURK.notify("~w~Doors are now ~g~Unlocked")
                        end
                    end
                end
            end)
                ...
                if K then
                    ...
                    if L ~= 0 then
                        ...
                        if M == 9 then
                            ...
                            tURK.notify("~r~Closed ~w~all doors.")
                        elseif M == 8 then
                            if r then
                                r = false
                                RollDownWindow(L, 0)
                                RollDownWindow(L, 1)
                                tURK.notify("Rolled windows ~r~Down")
                            else
                                r = true
                                RollUpWindow(L, 0)
                                RollUpWindow(L, 1)
                                tURK.notify("Rolled windows ~g~Up")
                            end
                        elseif IsVehicleDoorDamaged(L, N) then
                            tURK.notify("~r~Cannot shut this door when the door is damaged")
                        elseif GetVehicleDoorAngleRatio(L, N) == 0 then
                            SetVehicleDoorOpen(L, N, false, false)
                            tURK.notify(string.format("The ~b~%s ~w~door is now ~g~Open", h[M]))
                        else
                            SetVehicleDoorShut(L, N, false)
                            tURK.notify(string.format("The ~b~%s ~w~door is now ~r~Closed", h[M]))
                        end
                    end
                end
                k.door = M
            ...
            RageUI.Checkbox("Activate Speed Limiter","Select to turn on Speed Limiter",k.limiter,{RightBadge = RageUI.CheckboxStyle.Tick},function(I, K, J, O)
                if K then
                    if k.limiter then
                        ...
                        k.cruise = false
                        if not p then
                            local speed = GetEntitySpeed(tURK.getPlayerVehicle())
                            if convert(speed) < 10.0 then
                                p = true
                                limiter()
                            else
                                p = false
                                tURK.notify("~r~Alert~w~: Please slow down to enable the speed limiter.")
                            end
                        end
                    else
                ...
                if K then
                    ...
                    if GetIsVehicleEngineRunning(L) then
                        if L ~= 0 then
                            ...
                            tURK.notify("You've turned the ignition into the ~r~off ~w~position.")
                        end
                    else
                        if L then
                            SetVehicleEngineOn(L, true, false, true)
                            tURK.notify("You've turned the ignition to the ~g~on ~w~position.")
                        end
                    end
                end
            end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustPressed(1, 244) then
            if IsPedInAnyVehicle(tURK.getPlayerPed(), false) or RageUI.Visible(RMenu:Get("vehicle_menu", "main")) then
