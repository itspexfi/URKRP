local a = false
function func_ptt()
    if globalOnPoliceDuty and tURK.canAnim() and not noclipActive then
        local b = tURK.getPlayerPed()
        local c = tURK.getPlayerId()
        if a then
            ...
            if IsControlJustPressed(0, 19) and not IsPlayerFreeAiming(c) and IsUsingKeyboard(2) and not IsPedReloading(b) then
                ...
                tURK.loadAnimDict("random@arrests")
...
tURK.createThreadOnTick(func_ptt)
...
local function f(g)
    ...
    elseif not i and e then
        exports["pma-voice"]:cleURKxProximityMode()
        e = false
    end
end
tURK.createThreadOnTick(f)
