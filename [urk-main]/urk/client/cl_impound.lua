...
Citizen.CreateThread(function()
    tURK.createDynamicPed(a.ped.modelHash,a.ped.position,215.0,true,a.ped.animDict,a.ped.animName,30,false,false)
    tURK.addBlip(a.ped.position.x, a.ped.position.y, a.ped.position.z, 357, 81, "Vehicle Impound", 0.8, false)
    Wait(2000)
    local d = function()
        TriggerServerEvent('URK:getImpoundedVehicles')
        RageUI.Visible(RMenu:Get("urkimpound", "main"), true)
    end
    local e = function()
    ...
    tURK.createArea("vehicle_impound", a.ped.position, 3.0, 6, d, e, f, {})
end)
RMenu.Add("urkimpound","reasons",RageUI.CreateMenu("","~r~Impounding Vehicle...",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners", "urk_garageui"))
RMenu.Add("urkimpound","main",RageUI.CreateMenu("","~r~Your Impounded Vehicles",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners", "urk_garageui"))
RMenu.Add("urkimpound", "vehicle_information", RageUI.CreateSubMenu(RMenu:Get("urkimpound", "main")))
RMenu.Add("urkimpound", "pay_fine_confirmation", RageUI.CreateSubMenu(RMenu:Get("urkimpound", "vehicle_information")))
local g = 0
...
RageUI.CreateWhile(1.0, true, function()
    ...
    if RageUI.Visible(RMenu:Get('urkimpound', 'reasons')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.ButtonWithStyle("~g~Confirm Selection","",{RightLabel = "→→→"},true,function(i, k, j)
                if j then
                    ...
                    TriggerServerEvent("URK:impoundVehicle",b.owner_id,b.owner_name,b.vehicle,b.vehicle_name,m,b.vehicle_net_id)
                    RageUI.ActuallyCloseAll()
                end
            end,function()end)
    ...
    if RageUI.Visible(RMenu:Get('urkimpound', 'main')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.Separator("View your impounded vehicles here.")
                ...
                if c ~= nil and c ~= {} then
                    for o, p in pairs(c) do
                        ...
                        RMenu:Get("urkimpound", "vehicle_information"))
                    end
                else
    ...
    if RageUI.Visible(RMenu:Get('urkimpound', 'vehicle_information')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if g ~= 0 then
                ...
                end,RMenu:Get("urkimpound", "pay_fine_confirmation"))
            end
        end)
    end
    if RageUI.Visible(RMenu:Get('urkimpound', 'pay_fine_confirmation')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if g ~= 0 then
                ...
                RageUI.ButtonWithStyle("~g~Pay","Pay the fine",{RightLabel = "→→→"},true,function(i, k, j)
                    if j then
                        TriggerServerEvent("URK:releaseImpoundedVehicle", g.vehicle)
                    end
                end,RMenu:Get("urkimpound", "main"))
...
RegisterNetEvent("URK:receiveInfoForVehicleToImpound",function(r, s, t, u, v)
    b = {owner_id = tonumber(r), owner_name = s, vehicle = t, vehicle_name = u, vehicle_net_id = v}
end)

RegisterNetEvent("URK:receiveImpoundedVehicles",function(w)
    c = w
end)

RegisterNetEvent("URK:impoundSuccess",function(x, t, y, z, A, B, C, D)
    ...
    if O ~= vector3(0, 0, 0) and N ~= vector3(0, 0, 0) then
        tURK.loadModel("flatbed")
        tURK.loadModel("a_m_m_prolhost_01")
        P = tURK.spawnVehicle("flatbed", O.x, O.y, O.z, M, false, true, true)
        ...
        TriggerServerEvent("URK:deleteImpoundEntities",x,NetworkGetNetworkIdFromEntity(G),NetworkGetNetworkIdFromEntity(F))
        u = tURK.spawnVehicle(z, C.x, C.y, C.z, D, false, true, false)
        ...
        TriggerServerEvent("URK:awaitTowTruckArrival", x, R, T)
...
RegisterNetEvent("URK:attachVehToTowCl",function(x, W)
    local u = NetworkGetEntityFromNetworkId(x)
...
function tURK.impoundVehicleOptions(Z, _, x, a0)
    local a1 = g_vehicleHashToVehicleName[_]
    if X(a0) then
        tURK.notifyPicture2("polnotification","notification","The vehicle cannot be impounded with a person inside.","Metropolitan Police","Impound",nil,nil)
        return
    end
    local a2 = GetVehicleClass(a0)
    if a2 == 18 then
        tURK.notifyPicture2("polnotification","notification","Emergency vehicles cannot be impounded.","Metropolitan Police","Impound",nil,nil)
    elseif a.disallowedVehicleClasses[a2] then
        tURK.notifyPicture2("polnotification","notification","That vehicle is too large to be impounded.","Metropolitan Police","Impound",nil,nil)
    else
        TriggerServerEvent("URK:fetchInfoForVehicleToImpound", Z, a1, x)
        RageUI.Visible(RMenu:Get("urkimpound", "reasons"), true)
    end
end

function tURK.isVehicleImpounded(u)
    return c[u] ~= nil
end
...
exports("impound", tURK.impoundVehicleOptions)
exports("isImpounded", tURK.isVehicleImpounded)
