local a=module("URKVeh","cfg_vehiclemaxspeeds")
isInGreenzone = false
local cityCap = true
local b = false
local c = false
...
function tURK.areGreenzonesDisabled()
    return i
end
function tURK.setGreenzonesDisabled(p)
    i = p
end
n(
    function()
        for q, r in pairs(m) do
            if not r[5] and not tURK.isPurge() or r[5] and tURK.isPurge() then
...
n(
    function()
        local purgeActive = tURK.isPurge()
        while true do
            local t = tURK.getPlayerPed()
            local u = tURK.getPlayerCoords()
            for v, w in pairs(l) do
                if not w.isPurge and not purgeActive or w.isPurge and purgeActive then
                    ...
                    SetPlayerInvincible(tURK.getPlayerId(), false)
                    ...
                    SetNetworkVehicleAsGhost(tURK.getPlayerVehicle(), false)
                end
            end
            o(250)
        end
...
AddEventHandler("URK:onClientSpawn",function(y, z)
    if z then
        local A = function(B)
            inCityZone = true
        end
        ...
        tURK.createArea("cityzone",vector3(-225.30703735352, -916.74755859375, 31.216938018799),750.0,100,A,C,D,{})
    end
end)
local function E()
    local F = tURK.getPlayerCoords()
    local G = nil
    for H = 1, 25 do
    ...
    if G then
        local K, L = tURK.getPlayerVehicle()
        if K ~= 0 then
    ...
    while true do
        ...
        if not tURK.areGreenzonesDisabled() then
            ...
            if b or c then
                ...
                ClearPedBloodDamage(t)
                if usingDelgun then
                    tURK.setWeapon(t, "WEAPON_STAFFGUN", true)
                else
                    tURK.setWeapon(t, "WEAPON_UNARMED", true)
                end
                ResetPedVisibleDamage(t)
                ClearPedLastWeaponDamage(t)
            ...
            if b then
                ...
                local U, V = tURK.getPlayerCombatTimer()
                if U > 0 and V then
                    E()
                end
            end
...
RegisterCommand('togglecitycap', function()
    if tURK.getUserId() == 1 then
        cityCap = not cityCap
        tURK.notify('City Cap: '..(cityCap and "~g~Enabled" or "~r~Disabled"))
    end
end)