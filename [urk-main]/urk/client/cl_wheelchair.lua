RegisterCommand("wheelchair",function()
    if globalNHSOnDuty then
        local a = tURK.loadModel("prop_wheelchair_01")
        local b = CreateObject(a, GetEntityCoords(tURK.getPlayerPed()), true)
        SetModelAsNoLongerNeeded(a)
    end
end,false)
RegisterCommand("removewheelchair",function()
    if globalNHSOnDuty then
        local b = GetClosestObjectOfType(GetEntityCoords(tURK.getPlayerPed()), 10.0, GetHashKey("prop_wheelchair_01"))
...
function SitOnWheelchair(x)
    ...
    AttachEntityToEntity(tURK.getPlayerPed(), x, 0, 0, 0.0, 0.4, 0.0, 0.0, 180.0, 0.0, false, false, false, false, 2)
    local z = GetEntityHeading(x)
    while IsEntityAttachedToEntity(tURK.getPlayerPed(), x) do
        Citizen.Wait(5)
        if IsPedDeadOrDying(tURK.getPlayerPed()) then
            DetachEntity(tURK.getPlayerPed(), true, true)
        end
        if not IsEntityPlayingAnim(tURK.getPlayerPed(), "missfinale_c2leadinoutfin_c_int", "_leadin_loop2_lester", 3) then
            TaskPlayAnim(tURK.getPlayerPed(),"missfinale_c2leadinoutfin_c_int","_leadin_loop2_lester",8.0,8.0,-1,69,1,false,false,false)
        end
        local A = false
        ...
        if IsControlJustPressed(0, 73) or IsControlJustPressed(0, 177) then
            ...
            SetEntityCoords(tURK.getPlayerPed(), B, C, D)
            A = true
        end
        if A and not NetworkHasControlOfEntity(x) and GetGameTimer() - w >= 200 then
            local E = NetworkGetNetworkIdFromEntity(x)
            if E > 0 then
                TriggerServerEvent("URK:syncWheelchairPosition", E, GetEntityCoords(x, true), GetEntityHeading(x))
                w = GetGameTimer()
            end
        end
    end
...
function PickUpWheelchair(x)
    ...
    TriggerServerEvent("URK:wheelchairAttachPlayer", F)
    while not NetworkHasControlOfEntity(x) or not IsEntityAttachedToEntity(x, tURK.getPlayerPed()) do
        Wait(0)
    end
    LoadAnim("anim@heists@box_carry@")
    Wait(0)
    while IsEntityAttachedToEntity(x, tURK.getPlayerPed()) do
        Citizen.Wait(5)
        if not IsEntityPlayingAnim(tURK.getPlayerPed(), "anim@heists@box_carry@", "idle", 3) then
            TaskPlayAnim(tURK.getPlayerPed(), "anim@heists@box_carry@", "idle", 8.0, 8.0, -1, 50, 0, false, false, false)
        end
        if IsPedDeadOrDying(tURK.getPlayerPed()) then
...
RegisterNetEvent("URK:wheelchairAttachPlayer",function(F, G)
