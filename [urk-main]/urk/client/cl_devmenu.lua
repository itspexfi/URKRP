RMenu.Add(
    "devmenu",
    "main",
    RageUI.CreateMenu("URK Dev Tools", "Developer Tools", tURK.getRageUIMenuWidth(), tURK.getRageUIMenuHeight())
)
RMenu.Add(
    "devmenu",
    "animations",
...
local z = module("URKFirearms", "cfg/weapons")
RageUI.CreateWhile(
    1.0,
    true,
    function()
            ...
            RageUI.DrawContent(
                ...
                function()
                    ...
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                tURK.clientPrompt(
                                    "Customization [JSON]",
                                    json.encode(tURK.getCustomization()),
                                    function()
                                    end
                                )
                            end
                        ...
                        function(A, B, C)
                            if C then
                                tURK.clientPrompt(
                                    ...
                                    "",
                                    function(D)
                                        tURK.setCustomization(json.decode(D))
                                    end
                                )
                            end
                        end
                                ...
                                tURK.clientPrompt(
                                    ...
                                    function(F)
                                        tURK.spawnVehicle(
                                            ...
                                            E.y,
                                            E.z,
                                            GetEntityHeading(tURK.getPlayerPed()),
                                            true,
                                            true,
                                            true
                                        )
                        ...
                        function(A, B, C)
                            if C then
                                ...
                                tURK.clientPrompt(
                                    ...
                                    "",
                                    function(G)
                                        local H = tURK.loadModel(tonumber(G))
                                        if H then
                                            local I =
                                                GetEntityCoords(PlayerPedId()) +
                                                GetEntityForwardVector(PlayerPedId()) * 2.0
                    ...
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                TriggerEvent("URK:toggleTrafficMenu")
                            end
                        end
                    )
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                local J = tURK.loadModel("a_f_m_beach_01")
                                local K = tURK.getPlayerCoords()
                    ...
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                local J = tURK.loadModel("s_m_m_pilot_02")
                                local K = tURK.getPlayerCoords()
                                CreatePed(1, J, K.x, K.y, K.z, 0.0, true, true)
                                SetModelAsNoLongerNeeded(J)
                            end
                    ...
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                for k, v in pairs(z.weapons) do
                                    tURK.giveWeapons({[k] = {ammo = 250}})
                                end
                            end
                        end
                    )
                                ...
                                if string.match(t, S[c]) then
                                    RageUI.ButtonWithStyle(
                                        ...
                                        function(A, B, C)
                                            if C then
                                                u(R)
                                                TaskPlayAnim(tURK.getPlayerPed(), R, S[c], 1.0, -1, -1, 1, 0, 0, 0, 0)
                                                RemoveAnimDict(R)
                                            end
                                        end
                                    )
                    ...
                    for c = 1, #U.anims do
                        RageUI.ButtonWithStyle(
                            ...
                            function(A, B, C)
                                if C then
                                    u(U.dict)
                                    TaskPlayAnim(tURK.getPlayerPed(), U.dict, U.anims[c], 1.0, -1, -1, 1, 0, 0, 0, 0)
                        ...
                        RageUI.ButtonWithStyle(
                            ...
                            function(A, B, C)
                                if C then
                                    ...
                                    RequestIpl(S[2])
                                    SetEntityCoords(tURK.getPlayerPed(), tURK.getPlayerCoords())
                                    local K = tURK.getPlayerCoords()
                                    local W = GetInteriorAtCoords(K.x, K.y, K.z)
                                    tURK.notify("[URK] Ipl ~g~" .. S[1] .. "(" .. S[2] .. ")" .. " ~s~loaded")
                                end
                            end
                        )
                    end
...
function tURK.gridPositionSaver(aa, L, M)
    tURK.notify("pick 1st coord, press Y when done")
    ...
    Wait(250)
    tURK.notify("pick 2nd coord, press Y when done")
    while not IsControlJustPressed(0, 246) do
        Wait(0)
    end
    local ac = GetEntityCoords(PlayerPedId())
    Wait(250)
    tURK.notify("pick 3rd coord, press Y when done")
    while not IsControlJustPressed(0, 246) do
        Wait(0)
    end
    ...
    for c = 0, M - 1, 1 do
        for af = 0, L - 1, 1 do
            ...
            tURK.loadModel("a_m_m_fatlatin_01")
            local aj = tURK.spawnVehicle(aa, ai.x, ai.y, ai.z, heading, false, false, false)
    ...
    function()
        while true do
            ...
            if a then
                ...
                tURK.DrawText(0.32, 0.00, "~r~X:~s~ " .. roundx, 0.4)
                tURK.DrawText(0.38, 0.00, "~r~Y:~s~ " .. roundy, 0.4)
                tURK.DrawText(0.445, 0.00, "~r~Z:~s~ " .. roundz, 0.4)
                heading = GetEntityHeading(ak)
                roundh = tonumber(string.format("%.2f", heading))
                tURK.DrawText(0.50, 0.00, "~r~H:~s~ " .. roundh, 0.4)
                local al, am, an = table.unpack(GetEntityRotation(PlayerPedId(), 1))
                tURK.DrawText(0.38, 0.03, "~r~RX:~s~ " .. tonumber(string.format("%.2f", al)), 0.4)
                tURK.DrawText(0.44, 0.03, "~r~RY:~s~ " .. tonumber(string.format("%.2f", am)), 0.4)
                tURK.DrawText(0.495, 0.03, "~r~RZ:~s~ " .. tonumber(string.format("%.2f", an)), 0.4)
                speed = GetEntitySpeed(PlayerPedId())
                rounds = tonumber(string.format("%.2f", speed))
                tURK.DrawText(0.40, 0.92, "~r~Player Speed: ~s~" .. rounds, 0.4)
                health = GetEntityHealth(PlayerPedId())
                tURK.DrawText(0.40, 0.95, "~r~Player Health: ~s~" .. health, 0.4)
                camRotX = GetGameplayCamRot(2).x
                tURK.DrawText(0.36, 0.88, "~r~CR X: ~s~" .. tonumber(string.format("%.2f", camRotX)), 0.4)
                camRotY = GetGameplayCamRot(2).y
                tURK.DrawText(0.44, 0.88, "~r~CR Y: ~s~" .. tonumber(string.format("%.2f", camRotY)), 0.4)
                camRotZ = GetGameplayCamRot(2).z
                tURK.DrawText(0.51, 0.88, "~r~CR Z: ~s~" .. tonumber(string.format("%.2f", camRotZ)), 0.4)
                ...
                if IsPedInAnyVehicle(PlayerPedId(), 1) then
                    ...
                    tURK.DrawText(0.015, 0.76, "~r~Engine Health: ~s~" .. vehenground, 0.4)
                    tURK.DrawText(0.015, 0.73, "~r~Body Health: ~s~" .. vehbodround, 0.4)
                    tURK.DrawText(
                        0.015,
                        0.70,
                        "~r~Vehicle Fuel: ~s~" ..
...
function tURK.createDebugObject(Q)
    local aX = tURK.loadModel(Q)
