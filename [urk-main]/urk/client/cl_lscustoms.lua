...
local function DriveOutOfGarage(pos)
	Citizen.CreateThread(function()
		...
		local ok, veh_type, model = tURK.getNearestOwnedVehicle(1)
		myveh.model = model
...
AddEventHandler("URK:onClientSpawn",function(D, E)
    if E then
        ...
        for q,h in pairs(garages)do 
            tURK.addBlip(h.inside.position.x,h.inside.position.y,h.inside.position.z,72, nil, "LS Customs")
        end 
	end
end)
...
function tURK.isInsideLsCustoms()
    return inside
end
