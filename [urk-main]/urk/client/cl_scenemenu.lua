RMenu.Add("urkscenemenu","main",RageUI.CreateMenu("","~b~URK Traffic Scene Menu",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners","metpd"))
RMenu.Add("urkscenemenu","objects",RageUI.CreateSubMenu(RMenu:Get("urkscenemenu", "main"),"","~b~Spawn Objects",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners","metpd"))
RMenu.Add("urkscenemenu","speedZone",RageUI.CreateSubMenu(RMenu:Get("urkscenemenu", "main"),"","~b~Set a speed zone",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners","metpd"))
RMenu.Add("urkscenemenu","spikes",RageUI.CreateSubMenu(RMenu:Get("urkscenemenu", "main"),"","~b~Set a spike trap",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners","metpd"))
local b = {}
local c = {}
...
RageUI.CreateWhile(1.0, true, function()
    ...
    if RageUI.Visible(RMenu:Get('urkscenemenu', 'speedZone')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.ButtonWithStyle("Create Speedzone",nil,{},true,function(K, L, M)
                if M then
                    if p == 0 then
                        tURK.notify("~r~Please set a radius")
                    elseif q == 0 then
                        tURK.notify("~r~Please set a speed")
                    else
                        TriggerServerEvent("URK:createSpeedZone", tURK.getPlayerCoords(), q + 0.0, p + 0.0)
                        tURK.notify("~b~Speed zone created")
                    end
                end
            end,nil)
            RageUI.ButtonWithStyle("Delete Speedzone",nil,{},true,function(K, L, M)
                if M then
                    local Q = tURK.getPlayerCoords()
                    local R = nil
                    local S = 250.0
                    ...
                    if R then
                        TriggerServerEvent("URK:deleteSpeedZone", R)
                        tURK.notify("~r~Speed zone removed")
                    end
                end
            end,nil)
        end)
    end
    if RageUI.Visible(RMenu:Get('urkscenemenu', 'spikes')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
...
RegisterNetEvent("URK:toggleTrafficMenu",function()
    RageUI.Visible(RMenu:Get("urkscenemenu", "main"), not RageUI.Visible(RMenu:Get("urkscenemenu", "main")))
    if globalOnPoliceDuty and tURK.hasPoliceCallsign() then
        RMenu:Get("urkscenemenu", "main"):SetSubtitle("~b~MPD~w~: " .. tURK.getPoliceCallsign() .. " - " .. tURK.getPoliceRank() .. " - " .. tURK.getPoliceName())
    end
end)
...
function deleteObject()
    for n, o in pairs(k) do
        ...
        local Y, Z, _ = table.unpack(GetEntityCoords(tURK.getPlayerPed(), true))
        if DoesObjectOfTypeExistAtCoords(Y, Z, _, 0.9, X, true) then
            ...
            if NetworkGetEntityIsNetworked(a0) and not NetworkHasControlOfEntity(a0) then
                local a1 = tURK.getNetId(a0, "deleteObject()")
                if a1 ~= 0 then
                    TriggerServerEvent("URK:requestSceneObjectDelete", a1)
                end
            else
                DeleteObject(a0)
            end
        ...
        for n, a2 in pairs(b) do
            if DoesEntityExist(a2) then
                if NetworkGetEntityIsNetworked(a2) then
                    ...
                    if a3 ~= 0 then
                        TriggerServerEvent("URK:requestSceneObjectDelete", a3)
                    end
                else
                    DeleteEntity(a2)
                end
...
function deleteSpike()
    local Q = GetEntityCoords(tURK.getPlayerPed())
    local a8 = getClosetSpike(Q)
    if a8 then
        TriggerServerEvent("URK:removeSpike", a8[2])
    end
end
RegisterNetEvent("URK:addSpike",function(Q, a7)
    c[a7] = {Q, a7}
end)
RegisterNetEvent("URK:deleteSpike",function(a7)
    c[a7] = nil
end)
local a9 = 1
Citizen.CreateThread(function()
...
Citizen.CreateThread(function()
    while true do
        local a4, ah = tURK.getPlayerVehicle()
        if a4 ~= 0 and ah then
            local ai = false
            local aj = 1000.0
...
RegisterNetEvent("URK:createSpeedZone",function(T, ak, q, p)
...
RegisterNetEvent("URK:deleteSpeedZone",function(T)
    local U = j[T]
    if U then
        RemoveBlip(U.blip)
