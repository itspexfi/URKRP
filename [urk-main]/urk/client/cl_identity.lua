...
local i=0
local U=false
RMenu.Add("identity","main",RageUI.CreateMenu("","~r~City Hall",0,100,'banners', 'urk_cityhall'))
    ...
    if true then 
        ...
        local i=function()
            if IsControlJustPressed(1,51)then 
                TriggerServerEvent('URK:getIdentity')
                RageUI.ActuallyCloseAll()
                RageUI.Visible(RMenu:Get("identity","main"),not RageUI.Visible(RMenu:Get("identity","main")))
            end 
        end
        for j,k in pairs(b)do 
            tURK.createArea("identity_"..j,k,1.5,6,g,h,i)
            tURK.addMarker(k.x,k.y,k.z-0.2,0.5,0.5,0.5,0,50,255,170,50,20,false,false,true)
        end 
    end 
end)

RegisterNetEvent("URK:gotCurrentIdentity")
AddEventHandler("URK:gotCurrentIdentity", function(firstname,lastname,age)
    j=firstname
    k=lastname
    i=age
end)

RegisterNetEvent("URK:gotNewIdentity")
AddEventHandler("URK:gotNewIdentity", function(firstname,lastname,age)
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('identity', 'main')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if j ~= "N/A" then
                RageUI.Separator("~r~Your current identity")
                ...
                RageUI.ButtonWithStyle("Change your Identity","£5000",{RightLabel="→→→"},true,function(l,m,n)
                    if n then 
                        RageUI.ActuallyCloseAll()
                        TriggerServerEvent('URK:getNewIdentity')
                    end 
                end)
            end
        end)
    ...
    if RageUI.Visible(RMenu:Get('identity', 'confirm')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.Separator("~r~Your new identity")
            ...
            RageUI.ButtonWithStyle("Yes","",{RightLabel="→→→"},true,function(j,k,l)
                if l then 
                    TriggerServerEvent("URK:ChangeIdentity", newfirstname, newlastname, tonumber(newage))
                    RageUI.ActuallyCloseAll()
                end 
...
RegisterNetEvent("URK:showIdentity",function(v, w, x, y, z, A, B, C, D)
    o = true
    ...
    local H = tURK.getFontId("Akrobat-ExtraLight")
    x = string.upper(x)
    y = string.upper(y)
    while o do
...
RegisterNetEvent("URK:hideIdentity",function()
    o = false
end)
