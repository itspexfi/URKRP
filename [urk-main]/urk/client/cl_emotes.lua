...
RMenu.Add("emotesmenu","mainmenu",RageUI.CreateMenu("","~r~Main Menu",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners", "urk_settingsui"))
RMenu.Add("emotesmenu","emotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "mainmenu"),"","~r~Emotes",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners", "urk_settingsui"))
RMenu.Add("emotesmenu","searchemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "mainmenu"),"","S~r~earch Emotes",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners", "urk_settingsui"))
RMenu.Add("emotesmenu","danceemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "emotes"),"","D~r~ance Emotes",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners", "urk_settingsui"))
RMenu.Add("emotesmenu","customemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "emotes"),"","C~r~ustom Emotes",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners", "urk_settingsui"))
RMenu.Add("emotesmenu","gunemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "emotes"),"","~r~Gun Emotes",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners", "urk_settingsui"))
RMenu.Add("emotesmenu","propemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "emotes"),"","~r~Prop Emotes",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners", "urk_settingsui"))
RMenu.Add("emotesmenu","sharedemotes",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "emotes"),"","~r~Shared Emotes",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners", "urk_settingsui"))
RMenu.Add("emotesmenu","walkingstyles",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "mainmenu"),"","~r~Walking Styles",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners", "urk_settingsui"))
RMenu.Add("emotesmenu","moods",RageUI.CreateSubMenu(RMenu:Get("emotesmenu", "mainmenu"),"","~r~Moods",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners", "urk_settingsui"))
local b = false
local c = ""
local d = {}
local e = 0
...
local function p(q)
    ...
    if not q and GetGameTimer() - e < 1500 then
        tURK.notify("~r~Emotes are being rate limited.")
        return
    end
    b = false
    f = nil
...
local function F(E)
    if tURK.getPlayerVehicle() ~= 0 then
        tURK.notify("~r~Can not use scenarios whilst in a vehicle.")
        return
    end
    local A = PlayerPedId()
    ...
    elseif E[1] == "MaleScenario" then
        if tURK.getModelGender() == "male" then
            ...
            tURK.notify("~r~This scenario is male only.")
        end
...
local function a6(E)
    if not tURK.canAnim() or tURK.isHandcuffed() then
        return true
    end
    local A = PlayerPedId()
    ...
    local a9 = tURK.getPlayerVehicle()
...
local function a7(E)
    if a6(E) then
        tURK.notify("~r~Can not this emote at this time.")
        return
    end
...
local function ag(ah)
    if a6() then
        tURK.notify("~r~Can not use shared emotes at this time.")
        return
    end
    local ai = a8()
    if ai ~= 0 then
        i = ah
        j = ai
        TriggerServerEvent("URK:sendSharedEmoteRequest", ai, ah)
    else
        tURK.notify("~r~No player is near by.")
    end
end
local function aj(E)
    Citizen.CreateThreadNow(function()
        local ak = E[1]
        tURK.loadAnimDict(ak)
    ...
    if RageUI.Visible(RMenu:Get('emotesmenu', 'searchemotes')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if foundMatch == false then
                emoteString = tURK.KeyboardInput("Enter Emote Name", "", 10)
...
local function aF(aC, aD, aE)
    if #aD < 1 then
        tURK.notify("~r~No emote name was specified.")
        return
    end
    ...
    elseif a.guns[ah] then
        ...
        tURK.notify("~r~Invalid emote name was specified.")
    end
end
...
RegisterCommand("walk",function(aC, aD, aE)
    if #aD < 1 then
        tURK.notify("~r~No walk name was specified.")
        return
    end
    local ah = aD[1]
    if not ah then
    ...
    if a.walks[ah] then
        ...
        tURK.notify("~r~Invalid walk name was specified.")
    end
end)
RegisterNetEvent("URK:sendSharedEmoteRequest",function(aG, ah)
    if a.shared[ah] and not a6() then
        k = ah
        l = aG
        m = GetGameTimer()
        tURK.notify("~y~Y~w~ to accept, ~r~L~w~ to refuse (~g~" .. a.shared[ah][3] .. "~w~)")
    end
end)
RegisterNetEvent("URK:receiveSharedEmoteRequest",function(ah)
    p(false)
    Citizen.Wait(300)
    a7(a.shared[ah])
end)
RegisterNetEvent("URK:receiveSharedEmoteRequestSource",function()
...
local function aT()
    if k then
        ...
        elseif IsControlJustPressed(1, 246) then
            if a6() then
                tURK.notify("~r~You can not use emotes at this time.")
            else
                TriggerServerEvent("URK:receiveSharedEmoteRequest", l, a.shared[k][4])
            end
            aK()
        end
    end
    if b and f and g then
        ...
        if not IsEntityPlayingAnim(PlayerPedId(), f, g, 3) or tURK.getPlayerCombatTimer() > 0 then
            p(true)
        end
    end
end
function tURK.playEmote(ah)
    if a.emotes[ah] then
        a7(a.emotes[ah])
    elseif a.dances[ah] then
        a7(a.dances[ah])
