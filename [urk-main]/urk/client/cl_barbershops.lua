    ...
    if E then
        ...
        local f = function()
            if IsControlJustPressed(1, 51) then
                TriggerEvent("URK:openBarbershop")
            end
        end
        for g, h in pairs(a) do
            tURK.createArea("barbershop_" .. g, h, 1.5, 6, d, e, f)
            tURK.addMarker(h.x, h.y, h.z - 0.2, 0.5, 0.5, 0.5, 0, 50, 255, 170, 50, 20, false, false, true)
            tURK.addBlip(h.x, h.y, h.z, 71, 13, "Barber Shop")
        end
    end
end)