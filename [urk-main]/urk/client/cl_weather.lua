...
function tURK.setTime(f, g, h)
    b = {h = f, m = g, s = h}
end
function tURK.overrideTime(f, g, h)
    b.h = f
    b.m = g
    b.s = h
    c = true
end
function tURK.cancelOverrideTimeWeather()
    c = false
end
function tURK.setWeather(i)
    a = i
end
...
tURK.createThreadOnTick(func_manageTimeAndWeather)
RegisterNetEvent("URK:setTime",function(f, g, h)
    tURK.setTime(f, g, h)
end)
RegisterNetEvent("URK:setWeather",function(i)
    tURK.setWeather(i)
end)
...
function voteWeather(C)
    if j then
        if not k then
            TriggerServerEvent("URK:vote", C)
            k = true
            tURK.notify("~g~Vote sent!")
        else
            tURK.notify("~r~You have already voted!")
        end
    else
        tURK.notify("~r~Vote not in progress, start a vote with /voteweather!")
    end
end
function resetVotes()
    m = {
...
RegisterNetEvent("URK:startWeatherVote",function()
    j = true
...
RegisterNetEvent("URK:voteStateChange",function(D)
    m[D] = m[D] + 1
end)
RegisterCommand("voteweather",function()
    TriggerServerEvent("URK:tryStartWeatherVote")
end,false)
