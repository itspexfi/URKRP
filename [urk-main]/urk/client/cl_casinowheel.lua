...
RMenu:Get("luckywheel", "casino"):SetSubtitle("~r~You may only spin the wheel once per restart.")
RegisterNetEvent("URK:frameworkCleanup")
AddEventHandler("URK:frameworkCleanup",function()
    DeleteEntity(b)
end)
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('luckywheel', 'casino')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("Spin the wheel of URK (50,000 Chips)","",{RightLabel = "→→→"},true,function(e, f, g)
                if g then
                    tURK.notify("~r~Biome's dad needs to fix it.")
                    --TriggerServerEvent("URK:requestSpinLuckyWheel")
                end
            end)
        end)
    end
...
Citizen.CreateThread(
    function()
        ...
        local i = tURK.loadModel("vw_prop_vw_luckywheel_02a")
        local j = tURK.loadModel(c)
...
function goToWheel()
    if not d then
        ...
        local D = false
        while not D do
            local E = GetEntityCoords(tURK.getPlayerPed())
        ...
        SetEntityHeading(tURK.getPlayerPed(), 10.9754)
        Wait(2000)
    ...
    function(D, E)
        if E then
            ...
            for K, L in pairs(a) do
                tURK.addBlip(L.entryPosition.x, L.entryPosition.y, L.entryPosition.z, 681, 0, "URK Wheel", 0.7, true)
                tURK.createArea("urkwheel_" .. K, L.entryPosition, 1.5, 6, F, H, I, {})
            end
        end
    end
)
RegisterNetEvent("URK:syncLuckyWheel")
AddEventHandler(
    "URK:syncLuckyWheel",
    function(M)
        Wait(900)
...
RegisterNetEvent("URK:spinLuckyWheel")
AddEventHandler(
    "URK:spinLuckyWheel",
    function()
        goToWheel()
...
RegisterNetEvent("URK:spinLuckyWheelReaction")
AddEventHandler(
    "URK:spinLuckyWheelReaction",
    function(N)
        local y = tURK.getPlayerPed()
