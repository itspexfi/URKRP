...
local b = 10
RegisterNetEvent("URK:beginBusJob",function()
    a.tempVehicle = tURK.spawnVehicle("bus",a.spawnVehicleVector.x,a.spawnVehicleVector.y,a.spawnVehicleVector.z,343,true,true,true)
    a.onJob = true
    tURK.notify("~g~Bus Job started, exit the bus station and head to the first bus stop.")
    while a.onJob do
...
RegisterNetEvent("URK:endBusJob",function()
...
RegisterNetEvent("URK:nextStopReachedBusJob",function(d)
    ...
    if a.tempMarker then
        tURK.removeMarker(a.tempMarker)
    end
    ...
    local B=tURK.loadModel(passengerPeds[a.stopNumber].pedModel)
    local coords = tURK.getPosition()
    local radius = 10.0
...
RegisterNetEvent("URK:setNextBusJobBlip",function(f)
    a.tempBlip = AddBlipForCoord(f.x, f.y, f.z)
    SetBlipSprite(a.tempBlip, 1)
    SetBlipRoute(a.tempBlip, true)
    a.tempMarker = tURK.addMarker(f.x, f.y, f.z, 1.2, 1.2, 1.2, 255, 0, 0, 125, 50, 0, true, true)
end)
RegisterNetEvent('URK:onClientSpawn')
AddEventHandler("URK:onClientSpawn",function(g,h)
    if h then
        local i = function(j)
        ...
        local l = function(j)
            if IsControlJustReleased(1, 38) and not a.onJob then
                TriggerServerEvent("URK:attemptBeginBusJob")
            end
        end
        tURK.addBlip(a.startVector.x, a.startVector.y, a.startVector.z, 106, 1, "Bus Driver Job")
        tURK.addMarker(a.startVector.x,a.startVector.y,a.startVector.z - 1,1.0,1.0,1.0,255,0,0,70,50,39,false,false,true)
        tURK.createArea("bus", a.startVector, 1.5, 6, i, k, l, {})
    end
end)
function alert(msg) 
