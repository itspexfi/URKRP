local function a()
    local b = GetResourceKvpString("urk_scope_horizontal")
    if b == nil or b == "" then
        return 50
    else
        return tonumber(b)
...
local function f()
    local b = GetResourceKvpString("urk_scope_vertical")
    if b == nil or b == "" then
        return 30
    else
        return tonumber(b)
...
local i = GetResourceKvpString("urk_scope_enabled") == "true"
local j = false

RMenu.Add("scope","main",RageUI.CreateMenu("", "Scope Settings", tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners","urk_settingsui"))
...
function tURK.doesCurrentWeaponHaveScope()
    local p = PlayerPedId()
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        if i and (j or tURK.doesCurrentWeaponHaveScope()) then
            if j then
