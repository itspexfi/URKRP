RMenu.Add("policedog","main",RageUI.CreateMenu("Dog Support Unit", "~r~Dog Management", tURK.getRageUIMenuWidth(), tURK.getRageUIMenuHeight()))
AddEventHandler("URK:setPoliceOnDuty",function(a)
    if a then
        RegisterKeyMapping("k9attack", "Police Dog Attack", "keyboard", "INSERT")
    end
end)
AddEventHandler("URK:setPrisonGuardOnDuty",function(b)
    if b then
        RegisterKeyMapping("k9attack", "Police Dog Attack", "keyboard", "INSERT")
    end
end)
...
local i = "K9"
local j = "K9TARGET"
RegisterNetEvent("URK:policeDogMenu",function()
    if globalOnPoliceDuty and tURK.hasPoliceCallsign() then
        RMenu:Get("urkscenemenu", "main"):SetSubtitle("~r~MPD~w~: " .. tURK.getPoliceCallsign() .. " - " .. tURK.getPoliceRank() .. " - " .. tURK.getPoliceName())
    end
    if globalOnPrisonDuty and tURK.hasHmpCallsign() then
        RMenu:Get("urkscenemenu", "main"):SetSubtitle("~r~HMP~w~: " .. tURK.getHmpCallsign() .. " - " .. tURK.getHmpRank() .. " - " .. tURK.getHmpName())
    end
    RageUI.Visible(RMenu:Get("policedog", "main"), true)
end)
RegisterNetEvent("URK:sendClientRagdoll",function()
    SetPedToRagdoll(tURK.getPlayerPed(), 12000, 12000, 0, 0, 0, 0)
    showNotification("~y~~h~Alert~h~~s~: " .. "You have been bitten by a police dog.")
end)
RegisterNetEvent("URK:policeDogAttack",function()
...
local m = {breed = 1}
local function n()
    local o = GetResourceKvpString("urk_policedog_customisations")
...
local function r(s)
    ...
    local q = k[m.breed]
    p[q] = s
    SetResourceKvp("urk_policedog_customisations", json.encode(p))
end
...
function tpDog()
    local z = GetEntityCoords(tURK.getPlayerPed(), true)
                                    ...
                                    if M < 2.0 then
                                        ...
                                        for O, P in pairs(N) do
                                            if I == GetPlayerPed(P) then
                                                ...
                                                TriggerServerEvent("URK:performDogLog","Instructed dog to attack ID: " .. Q)
                                                TriggerServerEvent("URK:serverDogAttack", Q)
                                                H = true
                                                Wait(1000)
    ...
    while f.currentAction == d.SearchNearby and DoesEntityExist(f.handle) do
        ...
        for W, X in pairs(GetActivePlayers()) do
            ...
            if GetVehiclePedIsUsing(Y) == 0 and IsEntityVisible(Y) and #(GetEntityCoords(Y, true) - L) < 5.0 then
                ...
                local _ = tURK.clientGetUserIdFromSource(Z)
                if tURK.getJobType(_) == "" then
        ...
        if #V > 0 then
            ...
            TriggerServerEvent("URK:policeDogSniffPlayer", a0.source)
        end
        Citizen.Wait(5000)
    end
end
function putInVehicle()
    local a2 = tURK.getNearestVehicle(7.0)
    Citizen.Trace(a2)
...
function removeFromVehicle()
    if IsPedInAnyVehicle(tURK.getPlayerPed(), true) then
...
function newDog(q, a4)
    local a5 = tURK.getPlayerPed()
    if not IsPedInAnyPoliceVehicle(a5) then
        local a6 = tURK.loadModel(q)
        ...
        if DoesEntityExist(f.handle) then
            ...
            TriggerServerEvent("URK:performDogLog", "Created a new police dog")
        end
    end
end
function deleteDog()
    ...
    f.active = false
    f.handle = 0
    TriggerServerEvent("URK:performDogLog", "Deleted dog")
end
function dogFollow()
    if not f.active then
        return
    ...
    if f.currentAction ~= d.Follow then
        ClearPedTasks(f.handle)
        TaskFollowToOffsetOfEntity(f.handle, tURK.getPlayerPed(), 0.0, 0.0, 0.0, 7.0, -1, 10.0, true)
        f.currentAction = d.Follow
    else
        ClearPedTasks(f.handle)
        f.currentAction = d.Follow
...
function showNotification(ab, A)
    ...
    elseif ab == e.Info then
        AddTextComponentSubstringPlayerName("~r~~h~Info~h~~s~: " .. A)
    else
        AddTextComponentSubstringPlayerName(A)
    end
...
RegisterNetEvent("URK:policeDogIndicate",function(ad)
    ...
    notify("~r~Your dog starts indicating.")
    ClearPedTasks(f.handle)
    local ag = 0
