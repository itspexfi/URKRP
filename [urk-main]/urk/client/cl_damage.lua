...
local e = false
AddEventHandler("URK:hsSoundsOn",function()
    e = true
end)
AddEventHandler("URK:hsSoundsOff",function()
    e = false
end)
local f = false
AddEventHandler("URK:codHMSoundsOn",function()
    f = true
end)
AddEventHandler("URK:codHMSoundsOff",function()
    f = false
end)
...
function GameEventTriggered(i, j)
    if i == "CEventNetworkEntityDamage" then
        ...
        if m and l == PlayerPedId() and IsEntityAPed(k) and k ~= PlayerPedId() then
            TriggerEvent("URK:onPlayerKilledPed", k)
        end
        if k == PlayerPedId() then
            local t = tURK.getPedServerId(l)
            if n == 0 then
            ...
            CreateThread(function()
                ...
                if DecorExistOn(PlayerPedId(), "lasthp") then
                    ...
                    if u < v then
                        if c[r] then
                            if t then
                                TriggerServerEvent("URK:syncEntityDamage", u, v, t, true, m, n)
                            end
                        elseif d[r] then
                            if t then
                                TriggerServerEvent("URK:syncEntityDamage", u, v, t, false, m, n)
                            end
                        end
                    end
                    if m then
...
AddEventHandler("entityDamaged",function(w, x, n, y)
    ...
    if x == z and w ~= z and IsEntityAPed(w) and GetEntityHealth(w) > 105 and not IsEntityDead(w) then
        Citizen.Wait(0)
        TriggerEvent("URK:onPlayerDamagePed", w)
    end
end)
CreateThread(function()
...
local A = 0
RegisterNetEvent("URK:onEntityHealthChange",function(B, u, v, C)
    if f and (tURK.isPlusClub() or tURK.isPlatClub()) then
