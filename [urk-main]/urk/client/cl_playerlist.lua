...
function func_playerlistControl()
    if IsUsingKeyboard(2) then
        if IsControlJustPressed(0, 212) then
            a = not a
            TriggerServerEvent("URK:getPlayerListData")
            Wait(100)
...
tURK.createThreadOnTick(func_playerlistControl)
RegisterNUICallback("closeURKPlayerList",function(j, k)
    SetNuiFocus(false, false)
end)
AddEventHandler("URK:onClientSpawn",function(h, l)
    if l then
        TriggerServerEvent("URK:getPlayerListData")
    end
end)

RegisterNetEvent("URK:gotFullPlayerListData",function(staff, police, nhs, lfb, hmp, civillians)
...
local u,v,x
RegisterNetEvent("URK:playerListMetaUpdate",function(t)
    u, v, x = table.unpack(t)
    SendNUIMessage({wipeFooterPlayerList = true})
    SendNUIMessage({appendToFooterPlayerList = '<span class="foot" style="color: rgb(255, 255, 255);">Server #1 | </span>'})
    SendNUIMessage({appendToFooterPlayerList = '<span class="foot" style="color: rgb(0, 255, 20);">Server uptime ' ..tostring(u) .. "</span>"})
    SendNUIMessage({appendToFooterPlayerList = '<span class="foot" style="color: rgb(255, 255, 255);">  |  Number of players ' ..tostring(v) .. "/" .. tostring(x).."</span>"})
end)
...
function sendFullPlayerListData()
    ...
    SendNUIMessage({setServerMetaData = '<img src="playerlist_images/urk.png" align="top" width="20px",height="20px"><span class="staff">' ..tostring(H) .. "</span>"})
    ...
    SendNUIMessage({appendToFooterPlayerList = '<span class="foot" style="color: rgb(255, 255, 255);">  |  Number of players ' ..tostring(v) .. "/" .. tostring(x).."</span>"})
    if H >= 1 then
        SendNUIMessage({appendToContentPlayerList = '<span id="playerlist_seperator_staff" style="color: rgb(255, 255, 255);">Staff</span>'})
    end
    for S, T in pairs(sortedPlayersStaff) do
        SendNUIMessage({appendToContentPlayerList = '<span class="username" style="color: rgb(255, 255, 255);">' ..tostring(sortedPlayersStaff[S].name) ..'</span><span class="job">' ..tostring(sortedPlayersStaff[S].rank) .. '</span><span class="playtime">' .. tostring(sortedPlayersStaff[S].hours) .. "hrs</span><br/>"})
    end
    if I >= 1 then
    ...
    for S, T in pairs(sortedPlayersCivillians) do
        SendNUIMessage({appendToContentPlayerList = '<span class="username" style="color: rgb(255, 255, 255);">' ..tostring(sortedPlayersCivillians[S].name) ..'</span><span class="job">' ..tostring(sortedPlayersCivillians[S].rank) .. '</span><span class="playtime">' .. tostring(sortedPlayersCivillians[S].hours) .. "hrs</span><br/>"})
    end
end
...
Citizen.CreateThread(function()
    while true do
        ...
        if u ~= nil and v ~= nil and x ~= nil then
            SetDiscordAppId(1123759808252747918)
            SetDiscordRichPresenceAsset('urk') 
            SetDiscordRichPresenceAssetText("discord.gg/urkfivem") 
            SetDiscordRichPresenceAssetSmall('urksmall')
            SetDiscordRichPresenceAssetSmallText('URK')
            SetRichPresence("[ID:" .. tostring(tURK.getUserId()) .. "] | " .. tostring(v) .. "/" .. tostring(x))
        end
        Wait(15000)
    end
end)