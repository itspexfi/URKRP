...
RegisterNetEvent("URK:useTheForceTarget")
AddEventHandler("URK:useTheForceTarget",function()
    FreezeEntityPosition(PlayerPedId(),true)
    Wait(2500)
    tURK.loadAnimDict("ragdoll@human")
    TaskPlayAnim(tURK.getPlayerPed(),"ragdoll@human","electrocute",3.0,1.0,-1,01,0,0,0,0)
    Wait(5000)
    SetEntityHealth(tURK.getPlayerPed(),0)
    FreezeEntityPosition(PlayerPedId(),false)
end)


RegisterNetEvent("CMG:zapFriendlySync")
AddEventHandler(
    "CMG:zapFriendlySync",
    function(q, r)
...
RegisterNetEvent("URK:useTheForceSync")
AddEventHandler("URK:useTheForceSync",function(q,r,s)
    if#(q-GetEntityCoords(PlayerPedId()))<25.0 then 
        TriggerEvent("urk:PlaySound", "unlimitedpower")
        local s={}
        local a=0
        local diffVector=r-q
        for t=1,10,1 do 
    ...
    if tURK.getUserId() == 1 then
        ...
        if j then
            ...
            if A ~= -1 then
                ...
                local t = {}
                local s = r - q
                TriggerServerEvent("URK:theForceSync", A, q, r, s)
                for u = 1, 10, 1 do
                    table.insert(t, q + vector3(s.x / 10 * u, s.y / 10 * u, s.z / 10 * u))
                end
                TriggerEvent("urk:PlaySound", "unlimitedpower")
                FreezeEntityPosition(PlayerPedId(), true)
                tURK.loadAnimDict("rcmbarry")
                TaskPlayAnim(tURK.getPlayerPed(),"rcmbarry","bar_1_attack_idle_aln",3.0,1.0,-1,01,0,0,0,0)
