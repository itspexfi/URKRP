...
AddEventHandler("URK:onClientSpawn",function(p, q)
    if q then
        tURK.addMarker(b.x, b.y, b.z, 0.7, 0.7, 0.5, 0, 125, 255, 125, 150, 20, false, false, true)
        tURK.addBlip(b.x, b.y, b, 378, 3, "Organ Heist Police")
        tURK.addMarker(c.x, c.y, c.z, 0.7, 0.7, 0.5, 255, 0, 0, 125, 150, 20, false, false, true)
        tURK.addBlip(c.x, c.y, c.z, 378, 1, "Organ Heist Criminals")
    end
end)
...
Citizen.CreateThread(function()
    while true do
        if f then
            ...
            if IsControlJustPressed(0, 38) then
                if not globalOnPoliceDuty then
                    if not g then
                        TriggerServerEvent("URK:joinOrganHeist")
                        Wait(100)
                    else
                        tURK.notify("~r~Already joined the Organ Heist!")
                    end
                else
                    tURK.notify("~r~You are police, please use the other entrance!")
                end
            end
        elseif e then
            drawNativeNotification("~r~Press ~INPUT_PICKUP~ to play the Organ Heist!")
            if IsControlJustPressed(0, 38) then
                if globalOnPoliceDuty then
                    ...
                    tURK.notify("~r~You are a civillian, please use the other entrance!")
                end
            end
        end
        ...
        if inGamePhase or inWaitingStage then
            DrawGTATimerBar("~r~Police:", table.count(i), 3)
            DrawGTATimerBar("~r~Criminals:", table.count(j), 2)
            drawNativeText("~r~Kill the enemy team and survive.")
            RemoveWeaponFromPed(tURK.getPlayerPed(), "WEAPON_MOLOTOV")
            RemoveWeaponFromPed(tURK.getPlayerPed(), "WEAPON_FLASHBANG")
            RemoveWeaponFromPed(tURK.getPlayerPed(), "WEAPON_SMOKEGRENADE")
            RemoveWeaponFromPed(tURK.getPlayerPed(), "WEAPON_BZGAS")
            RemoveWeaponFromPed(tURK.getPlayerPed(), "WEAPON_STUNGUN")
            local r = PlayerPedId()
...
RegisterNetEvent("URK:teleportToOrganHeist",function(v, w, x, y)
    local r = PlayerPedId()
    d = w
    canPlayAnim = false
    h = x
    ...
    if x == "civ" then
        ...
        tURK.createAtm("Organ Heist", k.atmLocation)
    elseif x == "police" then
        SetPedRelationshipGroupHash(PlayerPedId(), m)
    end
    for l, z in pairs(k.gunStores[h]) do
        tURK.createGunStore(z[1], z[2], z[3])
    end
    tURK.setFriendlyFire(false)
    DoScreenFadeOut(1000)
...
RegisterNetEvent("URK:addOrganHeistPlayer",function(A, x)
    if x == "civ" then
        j[A] = true
    elseif x == "police" then
        i[A] = true
...
RegisterNetEvent("URK:organHeistKillConfirmed",function(B)
    PlaySoundFrontend(-1, "Weapon_Upgrade", "DLC_GR_Weapon_Upgrade_Soundset", true)
    tURK.playScreenEffect("MP_Celeb_Win", 0.25)
    tURK.notify("~g~Killed " .. B .. " received £25,000")
end)
    ...
    while true do
        if inWaitingStage then
            ...
            if #(s - k.pastGate) < 3.0 then
                ...
                NetworkFadeInEntity(PlayerPedId(), 0)
                DoScreenFadeIn(1000)
                tURK.notify("~r~You got too far from the organ heist and have been teleported back.")
            end
        end
        if inGamePhase then
            local s = GetEntityCoords(PlayerPedId())
...
RegisterNetEvent("URK:startOrganHeist",function()
...
RegisterNetEvent("URK:removeFromOrganHeist",function(F)
    if i[F] then
        i[F] = nil
    end
    if j[F] then
...
RegisterNetEvent("URK:endOrganHeist",function()
    ...
    inGamePhase = false
    d = 600
    tURK.deleteAtm("Organ Heist")
    for l, z in pairs(k.gunStores[h]) do
        tURK.deleteGunStore(z[1])
    end
    h = ""
    k = nil
...
RegisterNetEvent("URK:endOrganHeistWinner",function(x)
    Wait(10000)
    winOrganHeist(x)
    local r = PlayerPedId()
    tURK.setFriendlyFire(true)
...
function tURK.setDeathInOrganHeist()
    inOrganHeist = false
    inWaitingStage = false
    inGamePhase = false
end
