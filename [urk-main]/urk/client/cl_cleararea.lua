...
RegisterNetEvent("URK:clearVehicles",function()
    for b, c in pairs(GetGamePool("CVehicle")) do
        if NetworkHasControlOfEntity(c) then
            if GetPedInVehicleSeat(c, -1) == 0 and c ~= GetVehiclePedIsIn(tURK.getPlayerPed(), true) then
...
RegisterNetEvent("URK:clearBrokenVehicles",function()
    for b, c in pairs(GetGamePool("CVehicle")) do
        if NetworkHasControlOfEntity(c) then
            if GetPedInVehicleSeat(c, -1) == 0 and c ~= GetVehiclePedIsIn(tURK.getPlayerPed(), true) then
