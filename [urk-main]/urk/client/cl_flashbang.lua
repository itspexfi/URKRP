...
RegisterNetEvent("URK:flashbangExplode",function(coords)
    if #(coords - tURK.getPlayerCoords()) <= b then
...
function explodeFlashbang(coords)
    ...
    TaskPlayAnim(tURK.getPlayerPed(),"anim@heists@ornate_bank@thermal_charge","cover_eyes_intro",-8.0,-8.0,1000,50,8.0,0,0,0)
...
function func_checkForFlashbang()
    if not a then
        if GetSelectedPedWeapon(tURK.getPlayerPed()) == GetHashKey("WEAPON_FLASHBANG") then
            a = true
        else
            a = false
        end
    else
        if IsPedShooting(tURK.getPlayerPed()) and not c then
            c = true
            Citizen.CreateThread(function()
                ...
                if e ~= 0 then
                    ...
                    AddExplosion(coords, 25, 0.0, 1.0, nil, true, true)
                    TriggerServerEvent("URK:flashbangThrown", f)
                end
            end)
...
tURK.createThreadOnTick(func_checkForFlashbang)
