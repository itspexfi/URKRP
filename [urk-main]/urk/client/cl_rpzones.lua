...
local function f()
    ...
    g.position=tURK.getPlayerCoords()
    g.radius=g.radius+0.0
    g.vehicles.speed=g.vehicles.speed+0.0
    TriggerServerEvent("URK:createRPZone",g)
end
...
local function h()
    ...
    if i then 
        TriggerServerEvent("URK:removeRPZone",i)
    end 
end
local inRPZone = false
RMenu.Add("rpzones","mainmenu",RageUI.CreateMenu("","~r~Main Menu",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners","urk_adminui"))
RMenu.Add("rpzones","weapons",RageUI.CreateSubMenu(RMenu:Get("rpzones","mainmenu"),"","~r~Weapons",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners","urk_adminui"))
RMenu.Add("rpzones","vehicles",RageUI.CreateSubMenu(RMenu:Get("rpzones","mainmenu"),"","~r~Vehicles",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners","urk_adminui"))
...
RegisterNetEvent("URK:createRPZone",function(g)
...
RegisterNetEvent("URK:removeRPZone",function(r)
...
local function G(g)
    local l,u=GetCurrentPedWeapon(tURK.getPlayerPed())
    if u~=0 then 
        t(g,u)
    end
    local y,H=tURK.getPlayerVehicle()
    if y~=0 and H then 
        A(g,y)
    end
...
tURK.createThreadOnTick(I)

function tURK.GetRPZoneInfo()
    local K=tURK.getPlayerCoords()
    for l,g in ipairs(d)do 
        if#(K-g.position)<g.radius then
            return g
