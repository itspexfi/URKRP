...
local function i(j, k, l)
    if k < a then
        ...
        if HasEntityClearLosToEntity(tURK.getPlayerPed(), l, 17) then
            return true
        end
    elseif g and tURK.isPlayerInSelectedGang(j) then
        if k < h then
            return true
        end
    end
                            ...
                            if b[o] then
                                ...
                                if i(j, b[o], l) then
                                    ...
                                    if NetworkIsPlayerTalking(o) then
                                        ...
                                        local q, r = tURK.isPlayerInSelectedGang(j)
                                        if g and q then
                ...
                if
                    t ~= m and
                        (IsEntityVisible(t) or
                            not tURK.clientGetPlayerIsStaff(tURK.clientGetUserIdFromSource(u)) and not tURK.inEvent())
                 then
...
Citizen.CreateThread(
    function()
        while true do
            g = tURK.hasGangNamesEnabled() and not tURK.isEmergencyService()
            for n, o in ipairs(GetActivePlayers()) do
                local k = b[o]
                local j = GetPlayerServerId(o)
                if k and i(j, k, GetPlayerPed(o)) and d then
                    local w = nil
                    if g and tURK.isPlayerInSelectedGang(j) then
                        w = tURK.getPlayerName(o)
                    else
...
function tURK.setPlayerNameDistance(k)
    if k == -1 then
...
function tURK.getPlayerNameDistance()
    return a
end
RegisterCommand(
    "farids",
    function(x, y, z)
        if tURK.getStaffLevel() > 2 and tURK.isStaffedOn() then
            local A = y[1]
            if A ~= nil and tonumber(A) then
                a = tonumber(A) + 000.1
                tURK.setPlayerNameDistance(a)
                tURK.notify("~g~Far ids distance set to " .. A .. "m")
            else
                tURK.notify("~r~Please enter a valid range! (/farids [range])")
            end
        end
    end,
    false
