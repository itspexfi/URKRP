...
local function b()
    ...
    TriggerServerEvent("URK:startThrowSmokeGrenade", c)
    Citizen.Wait(100)
    ...
    TriggerServerEvent("URK:endThrowSmokeGrenade", c, g)
end
local function h(i)
...
RegisterNetEvent("URK:displaySmokeGrenade",function(c, g)
    local l = {coords = g, isDeleted = false}
    tURK.createArea("smoke_" .. tostring(c),g,250.0,1000.0,k,m,function()end,l)
    Citizen.Wait(60000)
    tURK.removeArea("smoke_" .. tostring(c))
    if l.handle then
        StopParticleFxLooped(l.handle, false)
        l.handle = nil
        l.isDeleted = true
