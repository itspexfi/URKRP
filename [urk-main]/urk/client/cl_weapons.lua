...
function tURK.getWeapons()
    ...
    for b,c in pairs(a.weapons)do 
        if HasPedGotWeapon(f,c.hash)then
            ...
            --h.attachments=tURK.getAllWeaponAttachments(b)
            d[b]=h 
        end 
    end
    return d 
end
function tURK.removeAllWeapons()
    RemoveAllPedWeapons(tURK.getPlayerPed())
end
...
RegisterCommand("storecurrentweapon",function()
    if s+3000<GetGameTimer()then 
        ...
        if HasPedGotWeapon(PlayerPedId(),'WEAPON_PISTOL50')or HasPedGotWeapon(PlayerPedId(),'WEAPON_MACHINEPISTOL')then 
            ...
            TriggerServerEvent("URK:forceStoreSingleWeapon",g)
        end 
    else 
        tURK.notify("~r~Store weapons cooldown, please wait.")
    end 
end)
            ...
            if not table.has(X, k) then
                for k,v in pairs(a.weapons) do
                    if i == GetHashKey(k) then
                        TriggerServerEvent('URK:acType17', v.name)
                        Wait(60000)
                    end
                end
            end
