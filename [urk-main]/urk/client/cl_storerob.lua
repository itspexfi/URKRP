...
RegisterNetEvent("URK:updateStoreRobBlips",function(p)
    refreshStoreBlips(p)
end)
AddEventHandler("URK:onClientSpawn",function(D, E)
    if E then
        TriggerServerEvent("URK:getStoreRobBlips")
        while p == nil do
            Wait(0)
        end
        while true do
    ...
    if not IsPedDeadOrDying(x) then
        if IsEntityPlayingAnim(x, "mp_am_hold_up", "holdup_victim_20s", 3) then
            ...
            if not IsPedDeadOrDying(x) then
                ...
                local A = tURK.loadModel("prop_poly_bag_01")
                        ...
                        function()
                            while true do
                                ...
                                if DoesEntityExist(B) then
                                    if #(GetEntityCoords(tURK.getPlayerPed()) - GetEntityCoords(B)) <= 1.5 then
                                        PlaySoundFrontend(
                                            -1,
                                            "ROBBERY_MONEY_TOTAL",
                                        ...
                                        TriggerServerEvent("URK:pickupCashFromApu", v)
                                        DeleteObject(B)
                                        break
                                    end
                                else
...
Citizen.CreateThread(
    function()
        while true do
            local D = GetEntityCoords(tURK.getPlayerPed(), true)
            if r ~= nil then
                for v, w in pairs(r) do
...
function refreshStoreBlips(E)
    ...
    for F, G in pairs(p) do
        ...
        if G.beingrobbed == true then
            tempBlip = tURK.addBlip(H.x, H.y, H.z, 52, 1, "Robbable Store [BEING ROBBED]")
        else
            tempBlip = tURK.addBlip(H.x, H.y, H.z, 52, 2, "Robbable Store")
        end
...
Citizen.CreateThread(
    function()
        for v, w in pairs(r) do
            local I = tURK.loadModel(w.shopNpcModel)
            ...
            SetPedFleeAttributes(J, 0, 0)
            local K = tURK.loadModel(w.prop_safe)
            local L = tURK.loadModel(w.prop_door)
            local M = tURK.loadModel(w.money_prop)
            local N = tURK.loadModel(w.money_prop2)
            local O = tURK.loadModel(w.money_prop3)
            local P = tURK.loadModel(w.money_prop4)
        ...
        while true do
            local W = GetEntityCoords(tURK.getPlayerPed())
            for v, w in pairs(r) do
                        ...
                        if HasEntityClearLosToEntityInFront(Y, Z) and not IsPedDeadOrDying(Z) and #(GetEntityCoords(Y) - GetEntityCoords(Z)) <= 5.0 then
                            if not q then
                                if GetSelectedPedWeapon(tURK.getPlayerPed()) == GetHashKey("WEAPON_UNARMED") then
                                    tURK.notify("~r~You need a weapon in your hands to rob this store!")
                                else
                                    TriggerServerEvent("URK:initiateStoreRobbery", v)
...
RegisterNetEvent("URK:beginStoreRobbingAnimations",function(v)
...
RegisterNetEvent("URK:storeRobberyInProgress",function(_, v)
    q = _
    local a0 = GetEntityCoords(tURK.getPlayerPed())
    while q do
        local W = GetEntityCoords(tURK.getPlayerPed())
        if #(a0 - W) > 20 then
            TriggerServerEvent("URK:forceEndRobbery", v)
...
RegisterNetEvent("URK:resetStorePed",function(v)
    r[v].robberyInProgress = false
    local Z = r[v].shopNpcHandler
    if DoesEntityExist(Z) then
        DeleteEntity(Z)
    end
    local I = tURK.loadModel(r[v].shopNpcModel)
...
function RunMiniGame(v, ad, ae)
    ...
    while safeCrackingInProgress do
        ...
        elseif safeCrackingState == "cracking" then
            ...
            local ag = GetEntityHealth(tURK.getPlayerPed())
            if ag <= 102 or tURK.isHandcuffed() then
                ...
                if IsControlJustPressed(0, 172) then
                    if ah == a[af] then
                        ...
                        if IsSafeUnlocked(b, af) then
                            ...
                            safeCrackingInProgress = false
                            ClearPedTasksImmediately(tURK.getPlayerPed())
                        end
                    else
                        af = 1
                        b = InitSafeLocks(a)
...
function GetSafeDoorAnimOffsetPosition(aj, ak, al)
    ...
    return vector3(aj.x + aq, aj.y + ar, GetEntityCoords(tURK.getPlayerPed()).z)
end
function PlaySafeCrackIntroAnim(as, h)
    local at = "mini@safe_cracking"
    local au = "step_into"
    loadAnimDict(at)
    TaskPlayAnimAdvanced(tURK.getPlayerPed(), at, au, as.x, as.y, as.z, 0.0, 0.0, h, 8.0, 8.0, -1, 2, 0.7, 0, 0)
    Wait(0)
    Wait(1000)
    safeCrackingState = "cracking"
end
function HandleSafeDialMovement()
    local av = tURK.getPlayerPed()
    local at = "mini@safe_cracking"
    local aw = ""
    ...
    loadAnimDict(at)
    if not IsEntityPlayingAnim(tURK.getPlayerPed(), at, aw, 3) then
        TaskPlayAnimAdvanced(tURK.getPlayerPed(), at, aw, g.x, g.y, g.z, 0.0, 0.0, h, 8.0, 8.0, -1, 1, 1.0, 0, 0)
    end
end
function HandleIncorrectMovement(af, b)
    ...
    loadAnimDict(at)
    TaskPlayAnimAdvanced(tURK.getPlayerPed(), at, az, g.x, g.y, g.z, 0.0, 0.0, h, 8.0, 8.0, -1, 1, 1.0, 0, 0)
    d = "rotation.idle"
end
...
RegisterNetEvent("URK:syncOpenSafeDoor",function(v)
    local aJ = r[v].doorHandler
    OpenSafeDoor(aJ)
end)
RegisterNetEvent("URK:syncCloseSafeDoor",function(v)
    local aJ = r[v].doorHandler
    CloseSafeDoor(aJ)
end)
function EndMiniGame(v, ad)
    ...
    local at = "mini@safe_cracking"
    loadAnimDict(at)
    TaskPlayAnimAdvanced(tURK.getPlayerPed(),at,"door_open_succeed_stand",g.x,g.y,g.z,0.0,0.0,h,8.0,8.0,-1,2,0.3,0,0)
    FreezeEntityPosition(tURK.getPlayerPed(), false)
    Wait(2500)
    TriggerServerEvent("URK:syncOpenSafeDoor", v)
    safeCrackingState = "setup"
