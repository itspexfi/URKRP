...
local i = {
    ...
    {position = vector3(-1114.5859375, 4936.806640625, 218.38012695313), commissionRate = 0},
...
Citizen.CreateThread(
    function()
        local j = tURK.addBlip(-552.90362548828, 5326.1733398438, 72.599586486816, 403, 3, "Meth Trader")
        local k = tURK.addBlip(133.73462677002, -1305.0340576172, 29.268741607666, 403, 4, "Cocaine Trader")
        local l = tURK.addBlip(103.47916412354, -1937.1091308594, 19.803705215454, 140, 2, "Weed Trader")
        local m = tURK.addBlip(3569.8334960938, 3662.5893554688, 33.920593261718, 586, 1, "Heroin Trader")
        local n = tURK.addBlip(-1118.4926757813, 4926.1889648438, 218.35691833496, 150, 1, "Large Arms Trader")
        local o = tURK.addBlip(5065.6201171875, -4591.3857421875, 2.8652405738831, 150, 1, "Large Arms Trader")
        local p = tURK.addBlip(1317.0300292969, 4309.8359375, 38.005485534668, 604, 8, "LSD North Trader", 1.0)
        local q = tURK.addBlip(2485.7429199219, -430.13989257813, 92.992835998535, 604, 8, "LSD South Trader", 1.0)
        local r = {
            {
                title = "Weed Trader",
                type = "radius",
...
AddEventHandler(
    ...
    function(v, w)
        if w then
            TriggerServerEvent("URK:refreshTurfOwnershipData")
            TriggerServerEvent("URK:requestDrugPriceUpdate")
        end
    end
)
RegisterNetEvent(
    "URK:attackGangCapture",
    function(x, y)
        f = true
        a = y
...
RegisterNetEvent(
    "URK:turfDefenseInterrupt",
    function()
        e = false
    end
)
RegisterNetEvent(
    "URK:initiateGangCaptureCheck",
    function(y, B)
        b[y] = B
    end
)
RegisterNetEvent(
    "URK:gotTurfOwnershipData",
    function(C)
...
function tURK.isPlayerInTurf()
    for D, E in pairs(h) do
        if #(tURK.getPlayerCoords() - E.position) < E.distance then
            return true
        end
    end
    return false
    ...
    function(v, w)
        if w then
            ...
            local O = function(M)
                if IsControlJustPressed(1, 51) then
                    if not PlayerIsInGang then
                        tURK.notify("~r~You need to be in a gang to capture a turf!")
                    elseif not f then
                        TriggerServerEvent("URK:refreshTurfOwnershipData")
                        TriggerServerEvent("URK:checkTurfCapture", M.turfId)
                    end
                    Wait(10000)
                end
            end
            for F, P in pairs(h) do
                tURK.createArea("turf_" .. F, P.position, 1.5, 6, L, N, O, {turfId = F})
            end
            local Q = function(M)
                            ...
                            if T ~= nil and tonumber(T) then
                                ...
                                if T <= 25 and T >= 0 then
                                    ...
                                    if GetGameTimer() > K + 30000 then
                                        ...
                                        if F == 1 then
                                            TriggerServerEvent("URK:setNewWeedPrice", T)
                                        elseif F == 2 then
                                            TriggerServerEvent("URK:setNewCocainePrice", T)
                                        elseif F == 3 then
                                            TriggerServerEvent("URK:setNewMethPrice", T)
                                        elseif F == 4 then
                                            TriggerServerEvent("URK:setNewHeroinPrice", T)
                                        elseif F == 5 then
                                            TriggerServerEvent("URK:setNewLargeArmsCommission", T)
                                        elseif F == 6 then
                                            TriggerServerEvent("URK:setNewLSDNorthPrice", T)
                                        elseif F == 7 then
                                            TriggerServerEvent("URK:setNewLSDSouthPrice", T)
                                        end
                                    else
                                        tURK.notify("~r~Rate limit, please wait 30s to change the commission.")
                                    end
                                end
                            end
                        end
            ...
            for F, G in pairs(i) do
                tURK.createArea("turfmanager_" .. F, G.position, 1.5, 6, Q, R, S, {turfId = F, draw = false})
            end
        end
    end
)
RegisterNetEvent(
    "URK:captureOwnershipReturned",
    function(F, U, V)
        if not U then
            startGangCapture(F, h[F].position, V)
        else
            tURK.notify("~r~Turf already owned!")
        end
    end
)
function otherMembersStartGangCapture(x, y, W)
    if b[y] then
        ...
        Citizen.CreateThread(
            function()
                local tmpMarker = tURK.addMarker(W.x, W.y, W.z - 10, 75, 75, 75, 0, 200, 0, 50, 1000)
                while f do
                    local X = GetEntityCoords(tURK.getPlayerPed(), false)
                    local Y = #(X - W)
                    if Y >= 37.5 then
                        tURK.notify("~r~Turf capture cancelled, you got too far!")
                        f = false
                        TriggerServerEvent("URK:gangDefenseLocationUpdate", y, "Attackers", true)
                    elseif GetEntityHealth(tURK.getPlayerPed()) <= 102 then
                        tURK.notify("~r~Turf capture failed, you died")
                        f = false
                        TriggerServerEvent("URK:gangDefenseLocationUpdate", y, "Attackers", true)
                    end
                    Wait(1000)
                end
                tURK.removeMarker(tmpMarker)
            end
        )
        Citizen.CreateThread(
            function()
...
function startGangCapture(x, W, y)
    if turfData[x].owned then
        tURK.notify("~r~You already own this turf!")
        TriggerServerEvent("URK:failCaptureTurfOwned", x)
    else
        TriggerServerEvent("URK:initiateGangCapture", x, y)
        local timer = g
        while b[y] == nil do
            Wait(0)
        end
        if b[y] then
            ...
            Citizen.CreateThread(
                function()
                    tmpMarker = tURK.addMarker(W.x, W.y, W.z - 10, 75, 75, 75, 0, 200, 0, 50, 1000)
                    while f do
                        local X = GetEntityCoords(tURK.getPlayerPed(), false)
                        local Y = #(X - W)
                        if Y >= 37.5 then
...
function defendGangCapture(x, a, Z, _, a0)
    ...
    Citizen.CreateThread(
        function()
            local tmpMarker = tURK.addMarker(Z, _, a0 - 10, 75, 75, 75, 0, 200, 0, 50, 1000)
            while e do
                local X = GetEntityCoords(tURK.getPlayerPed(), false)
                local Y = #(X - vector3(Z, _, a0))
                if Y < 37.5 and GetEntityHealth(tURK.getPlayerPed()) > 102 then
                    TriggerServerEvent("URK:gangDefenseLocationUpdate", a, "Defenders", true)
                elseif GetEntityHealth(tURK.getPlayerPed()) > 102 then
                    TriggerServerEvent("URK:gangDefenseLocationUpdate", a, "Defenders", false)
                elseif GetEntityHealth(tURK.getPlayerPed()) <= 102 then
                    tURK.notify("~r~Turf defense failed, you died")
                    e = false
                    TriggerServerEvent("URK:gangDefenseLocationUpdate", a, "Defenders", false)
                else
                    TriggerServerEvent("URK:gangDefenseLocationUpdate", a, "Defenders", false)
...
function gangCaptureSuccess(x, a)
    ...
    TriggerServerEvent("URK:gangCaptureSuccess", a)
end
function gangDefenseSuccess()
    TriggerServerEvent("URK:gangDefenseSuccess", a)
end
