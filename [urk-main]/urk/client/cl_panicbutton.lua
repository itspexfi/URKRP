...
RegisterCommand(
    ...
    function()
        if globalOnPoliceDuty or globalNHSOnDuty or globalOnPrisonDuty or globalLFBOnDuty or PlayerIsInGang then
            if b.Cooling == 0 then
                local f = {}
                f.Player = tURK.getPlayerId()
                f.Ped = tURK.getPlayerPed()
                f.Name = tURK.getPlayerName(f.Player)
                ...
                if globalOnPoliceDuty then
                    TriggerServerEvent("URK:newPanic", f, 3)
                elseif globalNHSOnDuty then
                    TriggerServerEvent("URK:newPanic", f, 2)
                elseif globalOnPrisonDuty then
                    TriggerServerEvent("URK:newPanic", f, 4)
                elseif globalLFBOnDuty then
                    TriggerServerEvent("URK:newPanic", f, 5)
                elseif PlayerIsInGang then
                    TriggerServerEvent("URK:newGangPanic", f)
                end
...
RegisterNetEvent(
    "URK:returnPanic",
    function(g, f, h)
        local i = tURK.getUserId()
        if
            globalOnPoliceDuty or globalNHSOnDuty or globalOnPrisonDuty or globalLFBOnDuty and h == 5 or
                h == 6 and not tURK.isEmergencyService()
         then
            if h == 6 and tURK.isEmergencyService() then
                return
            end
