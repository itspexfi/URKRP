...
function tURK.GetClosestPlayer(ao)
    local ap=-1
    local aq=-1
...
function tURK.globalSurrenderring()
...
RegisterNetEvent("URK:startRobbingPlayer",function()
    D=GetGameTimer()
end)
RegisterNetEvent("URK:endRobbingPlayer",function()
    D=nil 
end)
...
Citizen.CreateThread(function()
    ...
    while true do
        if (IsDisabledControlPressed(1,19) or IsControlPressed(1, 19)) and (IsDisabledControlPressed(1,63) or IsControlPressed(1, 63)) and d()and not sittingAtBlackjackTable then
            if not a2 and tURK.globalNHSOnDuty() then 
                a2=true
                ...
                detachFlag=1
                boneIndex=11816
                local aa=tURK.GetClosestPlayer(3)
                if aa then 
                    local ab=GetPlayerServerId(aa)
                    if ab~=-1 then 
                        TriggerServerEvent("URK:animationSync",aa,a3,a4,a5,a6,a7,y,ab,a9,a8,controlFlagMe,controlFlagTarget,animFlagTarget,detachFlag,boneIndex)
                    end 
                end 
            else 
                a2=false
                ...
                if ac~=0 then 
                    TriggerServerEvent("URK:animationSyncStop",ac)
                end 
            end
            Wait(250)
        end
...
RegisterNetEvent("URK:animationSyncClStop")
AddEventHandler("URK:animationSyncClStop",function()
    ClearPedSecondaryTask(PlayerPedId())
    DetachEntity(PlayerPedId())
end)
RegisterCommand("baseball",function(f,g)
    ...
    if aa~=nil and not c()and GetEntityHealth(GetPlayerPed(aa))>102 and d()and canAnim()then 
        TriggerServerEvent("URK:animationSync",aa,a3,a4,a5,a6,a7,y,ac,a9)
...
RegisterCommand("baseball9", function(f, g)
	...
	local ac = GetPlayerServerId(aa)
	if aa ~= nil and (not c()) and GetEntityHealth(GetPlayerPed(aa)) > 102 and d() and canAnim() then
		TriggerServerEvent("URK:animationSync", aa, a3, a4, a5, a6, a7, y, ac, a9, a8, controlFlagMe, controlFlagTarget, animFlagTarget, detachFlag)
	end
end, false)
...
RegisterNetEvent("URK:animationSyncTarget")
AddEventHandler("URK:animationSyncTarget",function(ac,ad,ae,a6,a7,y,a9,a8,af,animFlagTarget,detachFlag,boneIndex)
    local e=PlayerPedId()
...
RegisterNetEvent("URK:animationSyncMe")
AddEventHandler("URK:animationSyncMe",function(ad,ah,a9,af,ai)
    local e=PlayerPedId()
    RequestAnimDict(ad)
    while not HasAnimDictLoaded(ad)do 
        Citizen.Wait(10)
...
local ah = false
RegisterNetEvent("URK:startShavingPlayer",function(ai)
	...
	tURK.loadAnimDict("misshair_shop@hair_dressers")
	...
	tURK.loadModel("prop_clippers_01")
...
RegisterNetEvent("URK:startBeingShaved",function(aq)
...
RegisterNetEvent("URK:playDelayedShave",function(au)
...
RegisterNetEvent("URK:setAsShaved",function(ay)
	ah = true
	Citizen.Wait(ay)
	ah = false
end)
