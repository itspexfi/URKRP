...
function tURK.isPlayerRappeling()
    return k
end
function tURK.isPlayerInPoliceHeli()
    return l
end
local r = (a + b) * 0.5
local s = 0
...
local function X(Y)
    ...
    tURK.DrawText(0.45, 0.9, "Model: " .. _ .. "\nPlate: " .. a0, 0.55)
end
local function a1()
    while true do
        if l then
            local a2 = tURK.getPlayerPed()
            local heli = tURK.getPlayerVehicle()
            ...
            while m and not IsEntityDead(a2) and GetVehiclePedIsIn(a2) == heli and IsHeliHighEnough(heli) do
                ...
                if o then
                    ...
                    if ai - p >= 500 then
                        ...
                        TriggerServerEvent("URK:updateSpotlight", ak)
                        p = ai
                    end
...
local av = {}
RegisterNetEvent("URK:updateSpotlight",function(aw, ax)
    local ay = GetPlayerFromServerId(aw)
    if ay == -1 then
        return
    end
    local az = GetPlayerPed(ay)
    if az == 0 or az == tURK.getPlayerPed() then
        return
    end
