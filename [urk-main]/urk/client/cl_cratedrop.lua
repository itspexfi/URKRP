...
local d
RegisterNetEvent("URK:crateDrop",function(e, f, g)
    -- e = vector3
    -- f = crate id
    -- g (if it's rig)
    ...
    DecorSetInt(a[f].crate, "URKACVeh", 955)
    ...
    DecorSetInt(a[f].parachute, "URKACVeh", 955)
    ...
    --AddOwnedExplosion(tURK.getPlayerPed(), y.x, y.y, y.z, 1, 0.0, true, false, 3.0)
    FreezeEntityPosition(a[f].crate, true)
    for h, i in pairs(c) do
...
RegisterNetEvent("URK:removeLootcrate",function(f)
    if a[f] then
...
RegisterNetEvent("URK:addCrateDropRedzone",function(f, e)
    tURK.loadModel("xs_prop_arena_crate_01a")
    a[f] = {}
...
RegisterNetEvent("URK:removeCrateRedzone",function()
    SetTimeout(300000,function()
        RemoveBlip(radiusBlip)
    end)
end)
