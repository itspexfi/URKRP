...
RMenu.Add("vkint","mainmenu",RageUI.CreateMenu("Teleporter", "Main Menu", tURK.getRageUIMenuWidth(), tURK.getRageUIMenuHeight()))
local b = nil
local function c(d)
    if tURK.getPlayerCombatTimer() > 0 then
        tURK.notify("~r~You can not teleport whilst you have a combat timer.")
        return
    end
...
Citizen.CreateThread(function()
    for f, d in pairs(a) do
        tURK.addMarker(d.position.x, d.position.y, d.position.z - 1, 1.0, 1.0, 1.0, 255, 255, 255, 170, 50, 27)
        tURK.createArea("vkint_" .. d.id,d.position,2.0,10.0,l,m,function()end,d)
    end
end)
