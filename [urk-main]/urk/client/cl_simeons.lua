...
RegisterNetEvent("URK:gotCarDealerInstances",function(m)
    a = m
end)
RegisterNetEvent("URK:gotCarDealerCategories",function(n)
    b = n
end)
RegisterNetEvent("URK:carDealerPurchased",function()
    DeleteVehicle(f)
    f = 0
    g = 0
    h = false
    local o = a[d].posVector
    SetEntityCoords(tURK.getPlayerPed(), o.x, o.y, o.z)
end)
local function p(q)
    if q ~= nil then
        d = q
...
local function s(t, u, v, w)
    ...
    if HasModelLoaded(x) then
        local z = CreateVehicle(x, u, v, w, GetEntityHeading(tURK.getPlayerPed()), false, false)
        DecorSetInt(z, "URKACVeh", 955)
        ...
        TaskWarpPedIntoVehicle(tURK.getPlayerPed(), z, -1)
        ...
        tURK.notify("~r~Could not load vehicle")
        return -1
    end
end
CreateThread(function()
    for A, B in pairs(l.simeonsInstances) do
        ...
        tURK.addBlip(C.x, C.y, C.z, D, E)
        tURK.addMarker(C.x, C.y, C.z, 0.7, 0.7, 0.5, G[1], G[2], G[3], 125, 50, F, true, true)
    end
end)
...
function isNearPreviewVector(J)
    if #(GetEntityCoords(tURK.getPlayerPed()) - J) < 5.0 then
        return true
    end
    return false
end
...
Citizen.CreateThread(function()
    while true do
        if a ~= nil then
            for A, B in pairs(a) do
                local C, D, E, F, G, H, I =B.posVector,B.blipId,B.blipColour,B.markerId,B.markerColourTable,B.permissionsTable,B.categorys
                c[A] = #(GetEntityCoords(tURK.getPlayerPed()) - C)
            end
        end
        Wait(250)
    end
end)
RMenu.Add("cardealer","mainmenu",RageUI.CreateMenu("", "", tURK.getRageUIMenuWidth(), tURK.getRageUIMenuHeight(), "banners",  "urk_simeonsui"))
RMenu:Get("cardealer", "mainmenu"):SetSubtitle("~r~Categories")
RMenu.Add("cardealer","categories",RageUI.CreateSubMenu(RMenu:Get("cardealer", "mainmenu"),"","~r~Vehicles",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight()))
RMenu.Add("cardealer","vehicle",RageUI.CreateSubMenu(RMenu:Get("cardealer", "categories"),"","~r~Options",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight()))
RMenu.Add("cardealer","confirm",RageUI.CreateSubMenu(RMenu:Get("cardealer", "vehicle"),"","~r~Are you sure?",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight()))
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if d ~= nil then
                ...
                RageUI.ButtonWithStyle("Test Drive " .. i[1],"",{RightLabel = "→→→"},true,function(N, O, P)
                    if P then
                        DeleteVehicle(GetVehiclePedIsIn(tURK.getPlayerPed()))
                        if not k then
                            RageUI.ActuallyCloseAll()
                            hash = GetHashKey(j)
                            ...
                            if HasModelLoaded(hash) then
                                ...
                                testDriveSeconds = 60
                                local S = GetEntityCoords(tURK.getPlayerPed())
                                DecorSetInt(testDriveCar, "URKACVeh", 955)
                                SetModelAsNoLongerNeeded(hash)
                                TaskWarpPedIntoVehicle(tURK.getPlayerPed(), testDriveCar, -1)
                                setVehicleFuel(testDriveCar, 100)
                                tURK.notify("~g~You have 1 minute to test drive this vehicle!")
                                for T = 0, 24 do
                                ...
                                SetTimeout(60000,function()
                                    if k then
                                        ...
                                        SetEntityCoords(tURK.getPlayerPed(), S.x, S.y, S.z)
                                        tURK.notify("~r~Test drive over!")
                                        p()
                                    end
                                end)
                                Citizen.CreateThread(function()
                                ...
                                Citizen.CreateThread(function()
                                    while k do
                                        local U = GetVehiclePedIsIn(tURK.getPlayerPed(), false)
                                        if U ~= nil then
    ...
    if RageUI.Visible(RMenu:Get('cardealer', 'confirm')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("Yes","",{RightLabel = "→→→"},true,function(N, O, P)
                if P then
                    TriggerServerEvent("URK:purchaseCarDealerVehicle", e, j)
                    TriggerServerEvent('URK:FetchCars', 'Standard Garage')
                end
...
tURK.createThreadOnTick(func_previewSimeonsVehicle)
