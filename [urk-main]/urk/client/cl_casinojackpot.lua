...
local u = {}
RMenu.Add("URKjackpot","instructions",RageUI.CreateMenu("", "", tURK.getRageUIMenuWidth(), tURK.getRageUIMenuHeight(), "banners", "urk_jackpotui"))
RMenu:Get("URKjackpot", "instructions"):SetSubtitle("~r~JACKPOT")
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('URKjackpot', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            end,RMenu:Get("URKjackpot", "instructions"))
        end)
    end
end)
Citizen.CreateThread(function()
    while true do
        if insideDiamondCasino then
            b = 1000
            a = -1
            local y = GetEntityCoords(tURK.getPlayerPed())
            for z = 0, 15, 1 do
                local A = t[z]
                local B = #(y - A)
                if B < b then
...
RegisterNetEvent("URK:sitAtJackpotChair",function(E)
    e = E
    sittingAtInsideTrack = true
    goToJackpotChair(E)
end)
RegisterNetEvent("URK:updatePlayerWinChance",function(F)
    k = F
end)
RegisterNetEvent("URK:updateTotalPot",function(G)
    r = G
end)
function goToJackpotChair(E)
    ...
    PlayFacialAnim(tURK.getPlayerPed(), "enter_left_readyidle_facial", H)
    RemoveAnimDict(H)
    local O
    local P
            ...
            if j then
                ...
                if u ~= nil then
                    for R, S in pairs(u) do
                        if S.user_id == tURK.getUserId() then
    ...
    while true do
        if f then
            ...
            if IsControlJustPressed(0, 201) then
                if h then
                    TriggerServerEvent("URK:setJackpotBet", p)
                else
...
function showHowToJackpot(a4)
    ...
    if a ~= -1 then
        RageUI.Visible(RMenu:Get("URKjackpot", "instructions"), a4)
    end
end
Citizen.CreateThread(function()
    while true do
        if a ~= -1 and b < 2 then
            if IsControlJustPressed(0, 38) then
                if d[a] == false then
                    TriggerServerEvent("URK:requestSitAtJackpot", a)
                end
            end
        end
        Wait(0)
...
AddEventHandler("URK:onClientSpawn",function(a5, a6)
    if a6 then
        TriggerServerEvent("URK:requestJackpotChairData")
    end
end)
RegisterNetEvent("URK:sendJackpotChairData",function(a7)
    d = a7
end)
Citizen.CreateThread(function()
    while true do
        if sittingAtInsideTrack and f then
            ...
            if IsControlJustPressed(0, 202) and not g then
                ...
                PlayFacialAnim(tURK.getPlayerPed(), "exit_left_facial", H)
                ...
                f = false
                showHowToJackpot(false)
                TriggerServerEvent("URK:leaveJackpotChair")
                e = -1
...
RegisterNetEvent("URK:rollJackpot",function(a8, a9, aa, ab, ac)
                                        ...
                                        SetTimeout(1500,function()
                                            SetTimeout(1000,function()
                                                SetTimeout(750,function()
                                                    ...
                                                    o = ab
                                                    if al then
                                                        if ac == tURK.getUserId() then
                                                            SendNUIMessage({transactionType = "playCasinoWin"})
                                                            TriggerServerEvent("URK:waitingOnWinConfirm")
                                                        else
                                                            if e ~= -1 then
...
function getTrackGenderAnimDict()
    local ax = GetEntityModel(tURK.getPlayerPed())
...
RegisterNetEvent("URK:successJackpotBet",function()
    f = false
    h = false
    j = true
    PlaySoundFrontend(-1, "Hack_Success", "DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS", 1)
end)
RegisterNetEvent("URK:beginJackpot",function()
    s = false
    l = 60
    m = true
...
RegisterNetEvent("URK:newJackpotBet",function(ay)
    table.insert(u, ay)
end)
RegisterNetEvent("URK:cleanupJackpot",function()
    u = {}
    if e ~= -1 then
        f = true
        local H = getTrackGenderAnimDict()
        ...
        NetworkAddPedToSynchronisedScene(tURK.getPlayerPed(), az, H, I, 2.0, -2.0, 5, 0, 1148846080, 0)
