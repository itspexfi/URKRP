...
RegisterCommand("togglemelee",function()
    if tURK.getUserId()==1 or tURK.getUserId()==2 then 
        c = not c
        tURK.notify('Melee Combat: '..(c and "~g~Enabled" or "~r~Disabled"))
    end 
end)
function tURK.enablePunching(d)
    c = d
end
...
AddEventHandler("URK:onClientSpawn",function(h, i)
    if i then
        ...
        for n, o in pairs(a) do
            tURK.createArea("boxing_" .. n, o.pos, o.radius, 6, j, l, m, {})
        end
    end
end)
...
AddEventHandler("URK:onClientSpawn",function(h, i)
    if i then
        ...
        for n, u in pairs(t) do
            tURK.addMarker(u.x, u.y, u.z, 1.0, 1.0, 1.0, 0, 0, 255, 100, 50, 27, false, false, true)
            tURK.createArea(n .. "_boxingGloves", u, 1.5, 6, j, l, m, {})
        end
    end
end)
local v = {}
function putGloves()
    local w = tURK.getPlayerPed()
    local x = tURK.loadModel("prop_boxing_glove_01")
