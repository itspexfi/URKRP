local a = module("URKFirearms", "cfg/cfg_weaponsonback")
...
local function g()
    ...
    local i = tURK.getWeapons()
    local j = false
    local k = globalOnPoliceDuty and tURK.getPlayerVehicle() ~= 0
    for l in pairs(i) do
        local m = GetHashKey(l)
        local n = a.weapons[m]
        if n then
            local o = (not k or n.type ~= "Heavy") and (tURK.isEmergencyService() or not isInGreenzone) and not tURK.isPlayerInAnimalForm()
