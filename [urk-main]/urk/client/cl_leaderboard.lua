local a = {
    ...
    rows = {
        {"Robbster", "~r~BLUE TEAM", "", false, 5, 2, 0, "1.4"},
        {"Thomas", "~r~BLUE TEAM", "", false, 7, 5, 0, "0.5"},
        {"Arthur", "~r~RED TEAM", "", false, 5, 0, 2, "0.5"},
        {"Robbster", "~r~BLUE TEAM", "", false, 5, 0, 2, "1.4"},
        {"Thomas", "~r~BLUE TEAM", "", false, 7, 5, 0, "0.5"},
...
RegisterCommand("leaderboardtest",function()
    if tURK.isDev() then
        local b = tURK.createLeaderboard(a)
        Wait(1000)
        tURK.closeLeaderboard(b)
    end
end,false)
local b = 1
local c = {}
...
function tURK.closeLeaderboard(b)
    c[b] = false
end
function tURK.createLeaderboard(m)
    local n = Scaleform("SC_LEADERBOARD")
    n.RunFunction("SET_DISPLAY_TYPE", {1})
    n.RunFunction("SET_MULTIPLAYER_TITLE", {"URK RP Event"})
...
function tURK.updateScoreboard(u, v, w)
    local x
    ...
    if u == -1 then
        ...
        if x then
            ...
            print("error: URK:updateScoreboard couldn't find row with values", u, v, w)
        end
    end
end
RegisterNetEvent(
    "URK:updateScoreboard",
    function(u, v, w)
        tURK.updateScoreboard(u, v, w)
    end
)
function tURK.setPlayerCanOpenLeaderboard(F)
    e = F
end
function tURK.clearLeaderboardData()
    d = {columns = {}, rows = {}}
end
function tURK.isLeaderboardSetup()
    return table.count(d.rows) > 0
end
function tURK.removePlayerFromLeaderboard(u)
