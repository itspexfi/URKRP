...
RMenu.Add("policeloadouts","main",RageUI.CreateMenu("","Please Select Division",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners","urk_gunstoreui"))
RMenu.Add("policeloadouts","confirm",RageUI.CreateSubMenu(RMenu:Get("policeloadouts", "main"),"","Confirm Selection",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight()))
    ...
    if RageUI.Visible(RMenu:Get('policeloadouts', 'confirm')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.Button("Confirm", nil, {RightBadge = "→→→"}, true, function(Hovered, Active, Selected)
                if Selected then
                    TriggerServerEvent('URK:selectLoadout', selectedLoadout)
...
RegisterNetEvent('URK:gotLoadouts')
AddEventHandler('URK:gotLoadouts', function(loadoutsTable)
    ...
    if q then
        ...
        local u = function(s)
            if IsControlJustPressed(1, 38) then
                TriggerServerEvent('URK:getPoliceLoadouts')
        ...
        for k,v in pairs(loadoutCoords) do
            tURK.createArea("police_loadouts_"..k, v, 1.5, 6, r, t, u, {locationId = k})
        end
    end
end)