RMenu.Add("nhs","main",RageUI.CreateMenu("NHS", "Status: ~b~Available", tURK.getRageUIMenuWidth(), tURK.getRageUIMenuHeight()))
RMenu.Add("nhs","equipment",RageUI.CreateSubMenu(RMenu:Get("nhs", "main"),"Equipment","Status: ~b~Available",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight()))
RMenu.Add("nhs","observations",RageUI.CreateSubMenu(RMenu:Get("nhs", "main"),"Observations","Status: ~b~Available",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight()))
RMenu.Add("nhs","lifepak",RageUI.CreateSubMenu(RMenu:Get("nhs", "main"),"Observations","Status: ~b~Available",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight()))
RMenu.Add("nhs","objects",RageUI.CreateSubMenu(RMenu:Get("nhs", "main"),"Objects","Status: ~b~Available",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight()))
RegisterKeyMapping("nhs", "NHS Menu", "keyboard", "U")
TriggerEvent("chat:addSuggestion", "/nhs", "Toggle the NHS Menu")
RegisterNetEvent("URK:beginRevive",function(a, b, c, d)
    print("nuser_id", b)
...
RegisterNetEvent("URK:attachLifepak",function(a, b, c, d)
            ...
            if e.active then
                ...
                if e.readyToRevive then
                    RageUI.Button("Shock Patient",nil,{RightLabel = ""},true,function(g, h, i)
                        if i then
                            TriggerServerEvent("URK:finishRevive", e.perm_id)
                            e.readyToRevive = false
                            TriggerServerEvent("URK:playNhsSound", "shockAdvisedCharging")
                        end
                    end)
                else
                    RageUI.Button("Detach from patient",nil,{RightLabel = ""},true,function(g, h, i)
                        if i then
                            ...
                            TriggerServerEvent("URK:playNhsSound", "threeBeeps")
                        end
                    end)
                end
            else
    ...
    if RageUI.Visible(RMenu:Get('nhs', 'lifepak')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                ...
                if e.readyToRevive then
                    RageUI.Button("Shock Patient",nil,{RightLabel = ""},true,function(g, h, i)
                        if i then
                            TriggerServerEvent("URK:finishRevive", e.perm_id)
                            e.readyToRevive = false
                            TriggerServerEvent("URK:playNhsSound", "carryOutShock")
                        end
                    end)
                end
            end
            RageUI.Button("Enable Metronome",nil,{RightLabel = "100-120bpm"},true,function(g, h, i)
                if i then
                    TriggerServerEvent("URK:playNhsSound", "metronome")
                end
            end)
...
function carryLifePak()
    ...
    f.active = true
    local l = tURK.loadModel("prop_lifepak")
...
function changeUpObs()
    Citizen.CreateThread(function()
        ...
        if not e.dead then
            ...
            if n == 3 then
                Wait(3000)
                TriggerServerEvent("URK:playNhsSound", "checkPatientAlarm")
            end
...
function beginObs(a, u, v, w, d)
    TriggerServerEvent("URK:playNhsSound", "threeBeeps")
    Wait(3000)
    TriggerServerEvent("URK:playNhsSound", "analysingHeartRythm")
    e = {}
    RMenu:Get("nhs", "observations"):SetSubtitle("~r~Patient: ~w~" .. d)
    RMenu:Get("nhs", "lifepak"):SetSubtitle("~r~Patient: ~w~" .. d)
    e.active = true
    e.user_id = u
    e.perm_id = v
    e.name = d
    ...
    if e.coma then
        ...
        if IsEntityDead(GetPlayerPed(u)) then
            ...
            if e.readyToRevive then
                ...
                5000)
                TriggerServerEvent("URK:playNhsSound", "pushToShock")
            end
        end
    end
end
function connectObsToPatient()
    TriggerServerEvent("URK:attachLifepakServer")
end
RegisterNetEvent("URK:clientPlayNhsSound",function(x, y)
    local z = GetEntityCoords(tURK.getPlayerPed())
    local A = #(z - x)
    if A <= 15 then
...
RegisterNetEvent("URK:returnRevive",function()
    TriggerServerEvent("URK:playNhsSound", "checkPatientAlarm")
    e.bloodPressure1 = math.random(20, 199)
    e.bloodPressure2 = math.floor(e.bloodPressure1 - e.bloodPressure1 / 2.9)
    if e.bloodPressure2 > e.bloodPressure1 then
    ...
    e.coma = false
    Wait(3000)
    TriggerServerEvent("URK:playNhsSound", "safeToTouch")
    Wait(12000)
    TriggerServerEvent("URK:playNhsSound", "threeBeeps")
end)
    ...
    if globalNHSOnDuty or globalLFBOnDuty then
        ...
        if E ~= GetHashKey("WEAPON_UNARMED") then
            if not table.has(B, E) then
                tURK.setWeapon(D, "WEAPON_UNARMED", true)
            end
        end
    end
end
...
local K = false
RegisterNetEvent("URK:attemptCPR",function()
    K = true
    tURK.loadAnimDict("missheistfbi3b_ig8_2")
    ...
    while K do
        if not IsEntityPlayingAnim(PlayerPedId(), "missheistfbi3b_ig8_2", "cpr_loop_paramedic", 3) then
            K = false
            TriggerServerEvent("URK:cancelCPRAttempt")
            break
        end
        Citizen.Wait(0)
    end
end)
RegisterNetEvent("URK:cancelCPRAttempt",function()
    K = false
    if tURK.canAnim() then
