...
local j = false
function tURK.isPlayerHidingInBoot()
    return j
end
local function k(vehicle, l)
    if tURK.isHandcuffed() or tURK.isTazed() then
        return false
    end
    ...
    if l and #(tURK.getPlayerCoords() - l) > 1.0 then
        return false
    end
    return true
end
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            local q = tURK.getPlayerVehicle()
            local r = tURK.getClosestVehicle(7.0)
            local s = GetVehicleClass(r)
            local t = GetVehicleDoorLockStatus(r)
            if q == 0 and a[s] and GetEntityHealth(tURK.getPlayerPed()) > 102 and not noclipActive then
                if r and r ~= 0 then
                    o = 0
                    ...
                    if w < 2 then
                        ...
                        if IsDisabledControlJustReleased(1, 137) and k(vehicle) then
                            if not b(vehicle) then
                                ...
                                tURK.startCircularProgressBar("",2000,nil,function()
                                end)
                                if k(vehicle, l) then
                                    if t <= 1 then
                                        tURK.setCanAnim(false)
                                        tURK.setWeapon(p, "WEAPON_UNARMED", true)
                                        j = true
                                        ...
                                        Wait(100)
                                        tURK.loadAnimDict("timetable@floyd@cryingonbed@base")
                                        ...
                                        while z and j do
                                            ...
                                            if IsDisabledControlPressed(0, 177) then
                                                if GetVehicleDoorLockStatus(r) <= 1 then
                                                    ...
                                                    tURK.notify("~r~Vehicle is locked, cannot get in boot.")
                                                end
                                            end
                                        ...
                                        tURK.notify("~r~Vehicle is locked, cannot get out of boot.")
                                    end
                                else
                                    tURK.notify("~r~You and the vehicle must remain stationary to get in.")
                                end
                            else
                                tURK.notify("~r~Someone is already in this boot.")
                            end
                        end
                    else
                        ClearHelp(true)
...
RegisterNetEvent("URK:removeHiddenInBoot",function()
    j = false
end)
