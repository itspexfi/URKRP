...
local b = module("URKFirearms", "cfg/weapons")
    ...
    if not a.InProgress and u then
        ...
        if i then
            ...
            if w ~= -1 then
                ...
                if x then
                    tURK.setWeapon(PlayerPedId(), foundWeapon, true)
                    a.InProgress = true
                    a.startTime = GetGameTimer()
                    a.targetSrc = w
                    TriggerServerEvent("URK:takeHostageSync", w)
                    a.type = "agressor"
                else
...
RegisterNetEvent("URK:takeHostageSyncTarget",function(y)
...
RegisterNetEvent("URK:takeHostageReleaseHostage",function()
    a.InProgress = false
    a.type = ""
    DetachEntity(PlayerPedId(), true, false)
    q("reaction@shove")
...
RegisterNetEvent("URK:takeHostageKillHostage",function()
    a.InProgress = false
    a.type = ""
...
RegisterNetEvent("URK:takeHostageCl_stop",function()
    a.InProgress = false
    a.type = ""
    ClearPedSecondaryTask(PlayerPedId())
    DetachEntity(PlayerPedId(), true, false)
    ...
    while true do
        if a.type == "agressor" then
            ...
            if IsEntityDead(PlayerPedId()) then
                ...
                RemoveAnimDict("reaction@shove")
                TriggerServerEvent("URK:takeHostageReleaseHostage", a.targetSrc)
            end
            ...
            elseif IsDisabledControlJustPressed(0, 74) and GetGameTimer() - a.startTime > 4000 then
                ...
                RemoveAnimDict("anim@gangops@hostage@")
                TriggerServerEvent("URK:takeHostageKillHostage", a.targetSrc)
                TriggerServerEvent("URK:takeHostageStop", a.targetSrc)
                Wait(100)
...
function tURK.takeHostageInProgress()
    return a.InProgress
end
