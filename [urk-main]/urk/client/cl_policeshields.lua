    ...
    while true do
        if a.active and a.id ~= 0 then
            ...
            if h ~= GetHashKey("WEAPON_UNARMED") and (not b[a.id].allowPistol or not table.has(c, h)) then
                tURK.setWeapon(g, "WEAPON_UNARMED", true)
            end
...
RMenu.Add("policeshields","main",RageUI.CreateMenu("Police Shields", "~r~Shield Management", tURK.getRageUIMenuWidth(), tURK.getRageUIMenuHeight()))
...
RegisterNetEvent("URK:toggleShieldMenu",function(l)
...
function removeShield(m)
    if DoesEntityExist(a.prop) then
        ...
        if m then
            tURK.notify("~g~Success~w~: Shield removed")
        end
    end
end
function createShield(n)
    ...
    local o = false
    local g = tURK.getPlayerPed()
    local p = tURK.getPlayerCoords()
    if a.active then
        removeShield(false)
    end
    a.id = n
    local q = tURK.loadModel(b[n].model)
    ...
    tURK.loadAnimDict(b[n].animDict)
