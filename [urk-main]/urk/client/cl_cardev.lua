RMenu.Add("cardev","mainmenu",RageUI.CreateMenu("", "", tURK.getRageUIMenuWidth(), tURK.getRageUIMenuHeight(), "banners", "urk_adminui"))
RMenu:Get("cardev", "mainmenu"):SetSubtitle("~r~Car Dev Menu")
RMenu.Add("cardev","vehiclemods",RageUI.CreateSubMenu(RMenu:Get("cardev", "mainmenu"),"","~r~Vehicle Mods",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight()))
RMenu.Add("cardev","vehiclemodindexes",RageUI.CreateSubMenu(RMenu:Get("cardev", "vehiclemods"),"","~r~Vehicle Mod Indexes",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight()))
RMenu.Add("cardev","extras",RageUI.CreateSubMenu(RMenu:Get("cardev", "mainmenu"),"","~r~Extras",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight()))
RMenu.Add("cardev","colours",RageUI.CreateSubMenu(RMenu:Get("cardev", "mainmenu"),"","~r~Colours",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight()))
local a = false
local b = false
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('cardev', 'mainmenu')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if b and tURK.getPlayerBucket() == 333 then
                RageUI.ButtonWithStyle("Spawn Vehicle (No Mods)","",{RightLabel = "→→→"},true,function(h, i, j)
                    if j then
                        tURK.clientPrompt("Spawncode:","",function(k)
                            if k ~= "" and tURK.getPlayerBucket() == 333 then
                                local l = tURK.loadModel(k)
                                TriggerServerEvent("URK:logVehicleSpawn", k, "/cardev")
                                local m = tURK.getPlayerCoords()
                                local n = tURK.spawnVehicle(l,m.x,m.y,m.z,GetEntityHeading(tURK.getPlayerPed()),true,true,true)
                                DecorSetInt(n, "URKACVeh", 955)
                                SetVehicleOnGroundProperly(n)
                                SetEntityInvincible(n, false)
                                SetPedIntoVehicle(tURK.getPlayerPed(), n, -1)
                ...
                RageUI.ButtonWithStyle("Delete Vehicle","",{RightLabel = "→→→"},true,function(h, i, j)
                    if j then
                        local o = GetVehiclePedIsIn(tURK.getPlayerPed(), false)
                        if NetworkGetEntityIsNetworked(o) then
                            local p = NetworkGetNetworkIdFromEntity(o)
                            if p ~= 0 then
                                DeleteEntity(aT)
                                --TriggerServerEvent("URK:cardevDeleteVehicle", p)
                            end
                        else
                            DeleteEntity(o)
                        end
                ...
                RageUI.ButtonWithStyle("Cycle through seats","",{RightLabel = "→→→"},true,function(h, i, j)
                    if j then
                        ...
                        if IsPedInAnyVehicle(q) then
                            ...
                            tURK.notify("~r~Not in a vehicle.")
                        end
                    end
                end)
                ...
                RageUI.Checkbox("Return to normal Universe","",b,{},function()
                    ...
                    b = true
                    TriggerServerEvent("URK:setCarDevMode", b)
                end,function()
                    b = false
                    TriggerServerEvent("URK:setCarDevMode", b)
                end)
    ...
    if RageUI.Visible(RMenu:Get('cardev', 'vehiclemods')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            for w, x in pairs(f) do
                if GetNumVehicleMods(tURK.getPlayerVehicle(), w) > 0 then
    ...
    if RageUI.Visible(RMenu:Get('cardev', 'vehiclemodindexes')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            if GetNumVehicleMods(tURK.getPlayerVehicle(), selectedModType) == 0 then
                RageUI.Text({message = "~r~No available mod indexes for this mod type for this vehicle."})
            else
                for w = 0, GetNumVehicleMods(tURK.getPlayerVehicle(), selectedModType) do
                    RageUI.ButtonWithStyle("Mod " .. w,"",{RightLabel = "→→→"},true,function(h, i, j)
                        if j then
                            SetVehicleModKit(tURK.getPlayerVehicle(), 0)
                            SetVehicleMod(tURK.getPlayerVehicle(), selectedModType, w)
                        end
                    end,RMenu:Get("cardev", "vehiclemodindexes"))
                end
            end
                        ...
                        if IsVehicleExtraTurnedOn(r, w) then
                            ...
                            RageUI.Button("Enable Extra " .. w,nil,{RightLabel = tostring(w)},true,function(h, i, j)
                                if j then
                                    ...
                                    TriggerEvent("URK:staffFixVehicle")
                                end
                            end)
                        end
                    end
        ...
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            for z, A in pairs(g) do
                RageUI.Button(A.name,nil,{},true,function(h, i, j)
                    if j then
                        SetVehicleColours(tURK.getPlayerVehicle(), A.colorindex, A.colorindex)
                    end
                end)
            end
        end)
...
RegisterCommand("cardev",function(C)
    if a and not tURK.isPurge() then
        B()
    end
end)
RegisterNetEvent("URK:setCarDev",function()
    a = true
end)
RegisterNetEvent("URK:setBucket",function(D)
    if b and D ~= 333 then
...
local function ae()
    if not b or tURK.getPlayerBucket() ~= 333 then
        if J then
            ac(false)
        end
        return
    ...
    L()
    local af = tURK.getFontId("Akrobat-ExtraBold")
