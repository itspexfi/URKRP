...
RMenu.Add('URKGarages', 'main', RageUI.CreateMenu("", "~r~URK Garages",tURK.getRageUIMenuWidth(), tURK.getRageUIMenuHeight(), "banners", "urk_garageui"))
RMenu.Add('URKGarages', 'owned_vehicles', RageUI.CreateSubMenu(RMenu:Get("URKGarages", "main")))
RMenu.Add('URKGarages', 'rented_vehicles', RageUI.CreateSubMenu(RMenu:Get("URKGarages", "main")))
RMenu.Add('URKGarages', 'rented_vehicles_manage', RageUI.CreateSubMenu(RMenu:Get("URKGarages", "rented_vehicles")))
RMenu.Add('URKGarages', 'rented_vehicles_information', RageUI.CreateSubMenu(RMenu:Get("URKGarages", "rented_vehicles_manage")))
RMenu.Add('URKGarages', 'owned_vehicles_submenu', RageUI.CreateSubMenu(RMenu:Get("URKGarages", "owned_vehicles")))
RMenu.Add('URKGarages', 'owned_vehicles_submenu_manage', RageUI.CreateSubMenu(RMenu:Get("URKGarages", "owned_vehicles_submenu")))
RMenu.Add('URKGarages', 'scrap_vehicle_confirmation', RageUI.CreateSubMenu(RMenu:Get("URKGarages", "owned_vehicles_submenu_manage")))
RMenu.Add('URKGarages', 'rented_vehicles_out_manage', RageUI.CreateSubMenu(RMenu:Get("URKGarages", "rented_vehicles")))
RMenu.Add('URKGarages', 'rented_vehicles_out_information', RageUI.CreateSubMenu(RMenu:Get("URKGarages", "rented_vehicles_out_manage")))
RMenu.Add('URKGarages', 'customfolders', RageUI.CreateSubMenu(RMenu:Get("URKGarages", "owned_vehicles")))
RMenu.Add('URKGarages', 'customfoldersvehicles', RageUI.CreateSubMenu(RMenu:Get("URKGarages", "customfolders")))
RMenu.Add('URKGarages', 'settings', RageUI.CreateSubMenu(RMenu:Get("URKGarages", "main")))
RMenu:Get('URKGarages', 'owned_vehicles'):SetSubtitle("~r~Garage Mangement Menu")
...
local function W(J)
    ...
    if V(J) then
        RageUI.Visible(RMenu:Get("URKGarages", "main"), true)
    end
end
local function X(J)
    RageUI.ActuallyCloseAll()
    RageUI.Visible(RMenu:Get("URKGarages", "main"), false)
end
CreateThread(function()
    ...
    for ad, ae in pairs(cfg.garageInstances) do
        tURK.createArea("garage_" .. ad,ae[2],1.5,6,a9,ab,ac,{garageType = ae[1], garageId = ad, position = ae[2]})
    end
    for _, af in pairs(Y) do
        tURK.addBlip(af.position.x, af.position.y, af.position.z, af.blipId, af.blipColor, af.name, 0.7, false)
    end
    for a1, ag in pairs(Z) do
        tURK.addMarker(ag.position.x,ag.position.y,ag.position.z,0.7,0.7,0.5,ag.colour[1],ag.colour[2],ag.colour[3],125,50,ag.markerId,true)
    end
end)
...
local function aj(E)
    ...
    CreateThread(function()
        ...
        if HasModelLoaded(ak) then
            ...
            SetEntityAlpha(tURK.getPlayerPed(), 0)
            FreezeEntityPosition(tURK.getPlayerPed(), true)
            SetEntityCollision(tURK.getPlayerPed(), false, false)
            SetEntityCollision(am, false, false)
            local an = GetEntityCoords(tURK.getPlayerPed())
            local ao = GetEntityRotation(tURK.getPlayerPed())
            local ap = CreateCam("DEFAULT_SCRIPTED_CAMERA", 1)
            local aq = vector3(an.x, an.y, an.z + 2.0) - GetEntityForwardVector(tURK.getPlayerPed()) * 4.0
            ...
            tURK.notify("~r~Failed to load vehicle.")
            return -1
        end
    end)
end
...
local fuelLevels = {}
RegisterNetEvent("URK:spawnPersonalVehicle",function(E, av, valetCalled, ax, plate, fuel)
    X()
    fuelLevels[E] = fuel
    if GetVehiclePedIsIn(tURK.getPlayerPed(), false) == d then
        DeleteEntity(d)
    end
    tURK.notify("~g~Loading vehicle, please wait.")
    local ay = globalVehicleOwnership[E]
    if ay == nil or not DoesEntityExist(ay[2]) then
        local ak = tURK.loadModel(E)
        if ak == nil then
            tURK.notify("~r~Vehicle does not exist, if you believe this is an error contact a Car Dev on discord.")
            return
        end
        globalVehicleModelHashMapping[ak] = E
        local as
        if valetCalled then
            local an = tURK.getPosition()
            ...
            if tostring(aE) ~= "vector3(0, 0, 0)" and tostring(aB) ~= "vector3(0, 0, 0)" then
                as = tURK.spawnVehicle(ak, aB.x, aB.y, aB.z + 0.5, aA or 0.0, false, true, true)
                tURK.loadModel("s_m_y_xmech_01")
                ...
                print("[URK] Spawned vehicle with spawncode: " .. tostring(E))
                DecorSetInt(as, "vRP_owner", tURK.getUserId())
                DecorSetInt(as, "vRP_vmodel", E)
                SetVehicleNumberPlateText(as, plate)
                globalVehicleOwnership[E] = {E, as}
                setVehicleFuel(as, fuelLevels[E])
                while tURK.getPlayerVehicle() ~= as do
                    Wait(100)
                end
                TriggerServerEvent("LSC:applyModifications", E, as)
                TriggerServerEvent("URK:spawnVehicleCallback", av, VehToNet(as))
                table.insert(m, as)
            end
        else
            local aG = ax or tURK.getPosition()
            as = tURK.spawnVehicle(ak, aG.x, aG.y, aG.z + 0.5, GetEntityHeading(PlayerPedId()), true, true, true)
            ar(as)
            print("[URK] Spawned vehicle with spawncode: " .. tostring(E))
            DecorSetInt(as, "vRP_owner", tURK.getUserId())
            DecorSetInt(as, "vRP_vmodel", E)
            ...
            setVehicleFuel(as, fuelLevels[E])
            TriggerServerEvent("LSC:applyModifications", E, as)
            TriggerServerEvent("URK:PayVehicleTax")
            TriggerServerEvent("URK:spawnVehicleCallback", av, VehToNet(as))
            table.insert(m, as)
            CreateThread(function()
                local aH = true
        ...
        while DoesEntityExist(as) do
            ...
            if fuelLevels[E] ~= aI then
                TriggerServerEvent("URK:updateFuel", E, math.floor(aI * 1000) / 1000)
                fuelLevels[E] = aI
        ...
        tURK.notify("This vehicle is already out.")
    end
end)
...
function func_previewGarageVehicle()
    if n then
        if IsControlJustPressed(0, 177) then
            ...
            SetEntityAlpha(tURK.getPlayerPed(), 255)
            FreezeEntityPosition(tURK.getPlayerPed(), false)
            SetEntityCollision(tURK.getPlayerPed(), true, true)
        end
    end
end
tURK.createThreadOnTick(func_previewGarageVehicle)
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('URKGarages', 'main')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            DeleteCar(veh)
            RageUI.ButtonWithStyle("Garages", f, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) 
                if Selected then
                    VehiclesFetchedTable = {}
                    TriggerServerEvent('URK:FetchCars', garage_type)
                    TriggerServerEvent('URK:getCustomFolders')
                    TriggerServerEvent("URK:getImpoundedVehicles")
                end
            end, RMenu:Get("URKGarages", "owned_vehicles"))
            RageUI.ButtonWithStyle("Store Vehicle", f, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) 
                if Selected then 
                    local aR = GetVehiclePedIsIn(tURK.getPlayerPed(), false)
            ...
            RageUI.ButtonWithStyle("Rent Manager", f, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) 
                if Selected then
                    TriggerServerEvent('URK:FetchRented')
                end
            end, RMenu:Get("URKGarages", "rented_vehicles"))
            RageUI.ButtonWithStyle("Settings", f, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) end, RMenu:Get("URKGarages", "settings"))
            RageUI.ButtonWithStyle("Spawn Last Vehicle", f, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) 
                if Selected then
                    local ay = globalVehicleOwnership[e]
                    if ay == nil or not DoesEntityExist(ay[2]) then
                        TriggerServerEvent("URK:spawnPersonalVehicle", e)
                    else
                        tURK.notify("~r~Vehicle is already out!")
                    end
                end
            end)
            RageUI.ButtonWithStyle("~y~Fuel all vehicles. (£25,000)", f, {RightLabel = "→→→"}, true,function(aO, aP, aQ)
                if aQ then
                    if tURK.isPlusClub() or tURK.isPlatClub() then
                        if not u then
                            TriggerServerEvent("URK:fuelAllVehicles")
                            u = true
                            SetTimeout(60000,function()
                                u = false
                            end)
                        else
                            tURK.notify("~r~You've done this too recently, try again later.")
                        end
                    else
    ...
    if RageUI.Visible(RMenu:Get('URKGarages', 'settings')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
    ...
    if RageUI.Visible(RMenu:Get('URKGarages', 'owned_vehicles')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            DeleteCar(veh)
            RentedVeh = false
            if not garageSettings.displayCustomFoldersinOwned then
                ...
                end, RMenu:Get("URKGarages", "customfolders"))
            end
            if garageSettings.displayCustomFoldersinOwned then
                for h,b in pairs(folders) do
                    ...
                    end, RMenu:Get("URKGarages", "customfoldersvehicles"))
                end
            end
            ...
            for T, K in pairs(aW) do  
                ...
                if V(J) and h == aX then
                    ...
                    end, RMenu:Get("URKGarages", "owned_vehicles_submenu"))
                end
            end
        end)
    end
    if RageUI.Visible(RMenu:Get('URKGarages', 'owned_vehicles_submenu')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            for J,K in pairs(b) do
                if J == g then
                    ...
                    for T, K in pairs(aY) do
                        ...
                        if E ~= "_config" then
                            if aZ then
                                if garageSettings.hideCustomFolderVehiclesFromOriginalGarages and tURK.isVehicleInAnyCustomFolder(E) then
                                else
                                    ...
                                    if tURK.isVehicleImpounded(E) then
                                        aT = aT .. " ~r~(IMPOUNDED)~w~"
                                    end
                                    RageUI.ButtonWithStyle(aT, b0 .. "Fuel " .. tostring(a_) .. "%",{RightLabel = "→→→"},true,function(aO, aP, aQ)
                                        ...
                                        if aQ then
                                            e = E
                                            f = aT
                                            TriggerServerEvent("URK:getVehicleRarity", E)
                                            RMenu:Get('URKGarages', 'owned_vehicles_submenu_manage'):SetSubtitle(aT)
                                        end
                                    end,RMenu:Get("URKGarages", "owned_vehicles_submenu_manage"))
                                end
                            end
                        end
                    end
    ...
    if RageUI.Visible(RMenu:Get('URKGarages', 'owned_vehicles_submenu_manage')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            local a_ = f
            if x[e] then
            ...
            RageUI.ButtonWithStyle('Spawn Vehicle', a_, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                if Selected then 
                    ...
                    if #m <= 5 then
                        ...
                        tURK.notify("~r~You may only take out a maximum of 5 vehicles at a time.")
                    end
                    RageUI.ActuallyCloseAll()
                end
            end)
            if not RentedVeh then
                        ...
                        if (GetOnscreenKeyboardResult()) then
                            ...
                            if result then 
                                ...
                                if string.upper(result) == 'CONFIRM' then
                                    TriggerServerEvent('URK:CrushVehicle', e) 
                                    Table_Type = nil
                                    RageUI.ActuallyCloseAll()
                                    RageUI.Visible(RMenu:Get('URKGarages', 'main'), true)  
                                end
                ...
                RageUI.ButtonWithStyle('Add to Custom Folder', a_, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                    if Selected then 
                        local folderName = tURK.KeyboardInput("Enter Folder Name", "", 25)
                        if folderName ~= nil then
                            if folders[folderName] ~= nil then
                                if not table.find(folders[folderName], e) then
                                    table.insert(folders[folderName], e)
                                    tURK.notify("~g~Added vehicle to custom folder.")
                                    TriggerServerEvent("URK:updateFolders", folders)
                                else
                                    tURK.notify("~r~Failed to add vehicle from folder, vehicle already in folder.")
                                end
                            else
                                tURK.notify("~r~Failed to add vehicle to folder, folder does not exist?")
                            end
                        end
                    end
                end)
                                ...
                                if table.find(folders[folderName], e) then
                                    for i = 1, #folders[folderName] do
                                        if folders[folderName][i] == e then
                                            ...
                                            TriggerServerEvent("URK:updateFolders", folders)
                                            tURK.notify("~g~Removed vehicle from custom folder.")
                                        end
                                    end
                                else
                                    tURK.notify("~r~Failed to remove vehicle from folder, vehicle not in folder.")
                                end
                            else
                                tURK.notify("~r~Failed to remove vehicle from folder, folder does not exist?")
                            end
                        end
                    end
                end)
                RageUI.ButtonWithStyle('View Remote Dashcam', a_, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                    if Selected then 
                        TriggerServerEvent("URK:viewRemoteDashcam", e)
                    end
                end)
                RageUI.ButtonWithStyle('Display Vehicle Blip', a_, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                    if Selected then 
                        TriggerServerEvent("URK:displayVehicleBlip", e)
                    end
                end)
            end
        end)
    end
    if RageUI.Visible(RMenu:Get('URKGarages', 'customfolders')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
                ...
                if Selected then
                    ...
                    if folderName ~= nil and folderName ~= '' then
                        if folders[folderName] == nil then
                            folders[folderName] = {}
                            TriggerServerEvent("URK:updateFolders", folders)
                            tURK.notify("~g~Created custom folder "..folderName)
                        else
                            tURK.notify("~r~Folder already exists.")
                        end
                    else
                        tURK.notify("~r~Invalid folder name.")
                    end
                end
            end)
                ...
                if Selected then
                    ...
                    if folderName ~= nil then
                        if folders[folderName] ~= nil and folderName ~= '' then
                            ...
                            tURK.notify("~g~Deleted custom folder "..folderName)
                            TriggerServerEvent("URK:updateFolders", folders)
                        else
                            tURK.notify("~r~Folder "..folderName.." does not exist.")
                        end
                    else
                        tURK.notify("~r~Invalid folder name.")
                    end
    ...
    if RageUI.Visible(RMenu:Get('URKGarages', 'customfoldersvehicles')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
    ...
    if RageUI.Visible(RMenu:Get('URKGarages', 'rented_vehicles')) then 
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            DeleteCar(veh)
            if next(RentedVehiclesOut) then
                ...
                end,RMenu:Get("URKGarages", "rented_vehicles_out_manage"))
            end
            if next(RentedVehiclesIn) then
                ...
                end,RMenu:Get("URKGarages", "rented_vehicles_manage"))
            end
        end)
    end
    if RageUI.Visible(RMenu:Get('URKGarages', 'rented_vehicles_out_manage')) then 
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            Hovered_Vehicles = nil 
            DeleteCar(veh)
            if next(RentedVehiclesOut) then
                for i,v in pairs(RentedVehiclesOut.vehicles) do
                    ...
                    end, RMenu:Get("URKGarages", "rented_vehicles_out_information"))
                end
            end
        end)
    end
    if RageUI.Visible(RMenu:Get('URKGarages', 'rented_vehicles_out_information')) then 
        RageUI.DrawContent({header = true, glare = false, instructionalButton = true}, function()
            if next(RentedVehiclesOut) then
                for i,v in pairs(RentedVehiclesOut.vehicles) do
                    if RentedVehicle == i then
                        ...
                        RageUI.ButtonWithStyle('Request Rent Cancellation', "~y~This will cancel the rent of " ..v[1], {}, true, function(Hovered, Active, Selected)
                            if Selected then 
                                TriggerServerEvent("URK:CancelRent", i, v[1], 'owner')
                            end
                        end)
                    end
                end
    ...
    if RageUI.Visible(RMenu:Get('URKGarages', 'rented_vehicles_manage')) then 
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            Hovered_Vehicles = nil 
            DeleteCar(veh)
            if next(RentedVehiclesIn) then
                for i,v in pairs(RentedVehiclesIn.vehicles) do
                    ...
                    end, RMenu:Get("URKGarages", "rented_vehicles_information"))
                end
            end
        end)
    end
    if RageUI.Visible(RMenu:Get('URKGarages', 'rented_vehicles_information')) then 
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            Hovered_Vehicles = nil 
            DeleteCar(veh)
            if next(RentedVehiclesIn) then
                for i,v in pairs(RentedVehiclesIn.vehicles) do
                    if RentedVehicle == i then
                        ...
                        RageUI.ButtonWithStyle('Request Rent Cancellation', "~y~This will cancel the rent of " ..v[1], {}, true, function(Hovered, Active, Selected)
                            if Selected then 
                                TriggerServerEvent("URK:CancelRent", i, v[1], 'renter')
                            end
                        end)
                    end
                end
...
RegisterNetEvent("URK:updateOwnedVehicles",function(J)
    globalOwnedPlayerVehicles = {}
    globalOwnedPlayerVehicles = J
end)

RegisterNetEvent('URK:ReturnFetchedCars')
AddEventHandler('URK:ReturnFetchedCars', function(table, fuellevels)
    VehiclesFetchedTable = table
    q = fuellevels
end)

RegisterNetEvent('URK:ReturnedRentedCars')
AddEventHandler('URK:ReturnedRentedCars', function(rentedin, rentedout)
    RentedVehiclesIn = rentedin
    RentedVehiclesOut = rentedout
end)

RegisterNetEvent('URK:sendFolders')
AddEventHandler('URK:sendFolders', function(foldertable)
    folders = foldertable;
end)

RegisterNetEvent('URK:CloseGarage')
AddEventHandler('URK:CloseGarage', function()
    DeleteCar(veh)
    Table_Type = nil
    RageUI.ActuallyCloseAll()
end)
...
AddEventHandler("URK:searchClient",function(c9)
    local y = tonumber(DecorGetInt(c9, "vRP_owner"))
    if y > 0 then
        tURK.loadAnimDict("missexile3")
        ...
        TriggerServerEvent("URK:searchVehicle", VehToNet(c9), y)
    else
        tURK.notify("~r~Vehicle is not owned by anyone")
    end
end)

local ce = {}
RegisterNetEvent("URK:lockpickClient",function(c9, cf)
    FreezeEntityPosition(tURK.getPlayerPed(), true)
    ...
    tURK.notify("~g~Lock Picking in progress, you can cancel by pressing [E].")
    Citizen.CreateThread(function()
        while cg do
            if not IsEntityPlayingAnim(tURK.getPlayerPed(),"anim@amb@clubhouse@tutorial@bkr_tut_ig3@","machinic_loop_mechandplayer",3) then
                TaskPlayAnim(tURK.getPlayerPed(),"anim@amb@clubhouse@tutorial@bkr_tut_ig3@","machinic_loop_mechandplayer",8.0,-8.0,-1,1,0,false,false,false)
            end
            local cj = math.floor((GetGameTimer() - ci) / 60000 * 100)
            drawNativeText("Lock Picking - " .. cj .. "%")
            if IsControlJustPressed(0, 38) then
                tURK.notify("~r~Lock Picking cancelled.")
                cg = false
                ch = true
    ...
    if cf and not ch then
        ...
        local y = tonumber(DecorGetInt(c9, "vRP_owner"))
        if y > 0 then
            TriggerServerEvent("URK:lockpickVehicle", GetEntityModel(c9), y)
        else
            tURK.notify("~r~Vehicle is not owned by anyone")
        end
        local ck = NetworkGetNetworkIdFromEntity(c9)
        if ck ~= 0 then
            TriggerServerEvent("URK:setVehicleLock", ck, false)
            tURK.notify("Vehicle unlocked.")
        end
    else
        tURK.notify("~r~Failed to lockpick vehicle.")
    end
end)

RegisterNetEvent("URK:playLockpickAlarm",function(cl)
    local N = NetToVeh(cl)
    if N then
        local bt = GetSoundId()
...
AddEventHandler("URK:verifyLockpick",function(c9)
    local y = tonumber(DecorGetInt(c9, "vRP_owner"))
    if y > 0 then
        if ce[c9] then
            TriggerServerEvent("URK:lockpickVehicle", GetEntityModel(c9), y)
        else
            TriggerServerEvent("URK:attemptLockpick", c9, VehToNet(c9))
        end
    else
        tURK.notify("~r~Vehicle owner is not online.")
    end
end)
...
function tURK.isVehicleInAnyCustomFolder(F)
...
RegisterNetEvent('URK:sendGarageSettings')
AddEventHandler('URK:sendGarageSettings', function()
    TriggerServerEvent("URK:refreshSimeonsPermissions")
...
RegisterNetEvent("URK:recieveRefreshedGaragePermissions",function(z)
    c = z
end)

RegisterNetEvent("URK:setVehicleRarity",function(F, G)
    x[F] = G
end)

RegisterCommand("callanambulance",function()
    tURK.notify("~y~CALL AN AMBULANCE")
    tURK.notify("~y~CALL AN AMBULANCE!")
    tURK.notify("~r~BUT NOT FOR ME.")
    SendNUIMessage({transactionType = "callanambulance"})
end)

RegisterCommand("car",function(bY, bZ, b_)
    if tURK.getStaffLevel() >= 6 and not tURK.isPurge() then
        local ap = GetEntityCoords(tURK.getPlayerPed())
        local c0 = vector3(-1341.9575195313, -3032.8686523438, 13.944421768188)
        local bz, bA, bB = table.unpack(GetOffsetFromEntityInWorldCoords(tURK.getPlayerPed(), 0.0, 3.0, 0.5))
        local ao = bZ[1]
        if ao == nil then
            tURK.notify("~r~No vehicle spawncode specified.")
            return
        end
        if ao == "demonhawkk" and tURK.getUserId() ~= 1 then
            tURK.teleport(-807.62481689453, 172.82191467285, 76.740547180176)
            jimmy()
            TriggerClientEvent('URK:chatFilterScaleform', source, 10, 'Do not spawn my car in nigger')
        else
            if #(ap - c0) < 600.0 or (tURK.getUserId() == 1 or tURK.getUserId() == 2 or tURK.getUserId() == 3) then
                TriggerServerEvent("URK:logVehicleSpawn", ao, "/car")
                local c1 = tURK.spawnVehicle(ao, ap.x, ap.y, ap.z, GetEntityHeading(tURK.getPlayerPed()), true, true, true)
                DecorSetInt(c1, "URKACVeh", 955)
                ...
                SetPedIntoVehicle(tURK.getPlayerPed(), c1, -1)
                SetModelAsNoLongerNeeded(GetHashKey(ao))
                SetVehRadioStation(c1, "OFF")
                Wait(500)
                SetVehRadioStation(c1, "OFF")
            else
                tURK.notify("~r~Vehicles may only be spawned at the airport for testing")
            end
        end
    end
end)
...
RegisterCommand("dv",function()
    if globalOnPoliceDuty or globalNHSOnDuty or globalLFBOnDuty or tURK.isStaffedOn() or tURK.isDev() or globalOnPrisonDuty and isPlayerNearPrison() then
        local aT = GetVehiclePedIsIn(tURK.getPlayerPed(), false)
        if NetworkHasControlOfEntity(aT) and (tURK.getStaffLevel() > 0 or GetEntitySpeed(aT) < 1.0) then
            DeleteEntity(aT)
        end
    end
end,false)

function tURK.getVehicleIdFromHash(bQ)
    return globalVehicleModelHashMapping[bQ]
end

function tURK.getVehicleInfos(am)
...
function tURK.getNetworkedVehicleInfos(b6)
...
function tURK.despawnGarageVehicle(bl)
    ...
    if ay then
        ...
        tURK.notify("Vehicle stored.")
    end
end
function tURK.getNearestVehicle(bm)
    local aG = tURK.getPosition()
    local bn, bo, bp = aG.x, aG.y, aG.z
    local bq = tURK.getPlayerPed()
...
function tURK.tryOwnNearestVehicle(bm)
    local am = tURK.getNearestVehicle(bm)
    if am then
        local x, b5 = tURK.getVehicleInfos(am)
        if x and x == tURK.getUserId() then
...
function tURK.fixeNearestVehicle(bm)
    local am = tURK.getNearestVehicle(bm)
    if IsEntityAVehicle(am) then
        SetVehicleFixed(am)
    end
end
function tURK.replaceNearestVehicle(bm)
    local am = tURK.getNearestVehicle(bm)
    if IsEntityAVehicle(am) then
        SetVehicleOnGroundProperly(am)
    end
end
function tURK.getVehicleAtPosition(bn, bo, bp)
    bn = bn + 0.0001
    bo = bo + 0.0001
    bp = bp + 0.0001
    local bz = StartExpensiveSynchronousShapeTestLosProbe(bn, bo, bp, bn, bo, bp + 4, 10, tURK.getPlayerPed(), 0)
    local R, S, bA, bB, bC = GetShapeTestResult(bz)
    return bC
end
function tURK.getNearestOwnedVehicle(bm)
    tURK.tryOwnNearestVehicle(bm)
    local bD
    local bE
    local bs
    local an = GetEntityCoords(tURK.getPlayerPed())
...
function tURK.getCurrentOwnedVehicle(bm)
    tURK.tryOwnNearestVehicle(bm)
    local bD
    local bE
    for F, G in pairs(globalVehicleOwnership) do
        local bF = #(GetEntityCoords(G[2], true) - GetEntityCoords(tURK.getPlayerPed()))
        if bF <= bm + 0.0001 then
            if not bD or bF < bD then
                bD = bF
                bE = F
...
function tURK.getAnyOwnedVehiclePosition()
    for F, G in pairs(globalVehicleOwnership) do
...
function tURK.getOwnedVehiclePosition(bl)
    local ay = globalVehicleOwnership[bl]
...
function tURK.getOwnedVehicleHandle(bl)
    local ay = globalVehicleOwnership[bl]
    if ay then
        return ay[2]
    end
end
function tURK.ejectVehicle()
    local bq = tURK.getPlayerPed()
    if IsPedSittingInAnyVehicle(bq) then
...
function tURK.isInVehicle()
    local bq = tURK.getPlayerPed()
    return IsPedSittingInAnyVehicle(bq)
end
function tURK.vc_openDoor(bl, bG)
    local ay = globalVehicleOwnership[bl]
    if ay then
        SetVehicleDoorOpen(ay[2], bG, 0, false)
    end
end
function tURK.vc_closeDoor(bl, bG)
    local ay = globalVehicleOwnership[bl]
    if ay then
        SetVehicleDoorShut(ay[2], bG)
    end
end
function tURK.vc_detachTrailer(bl)
    local ay = globalVehicleOwnership[bl]
    if ay then
        DetachVehicleFromTrailer(ay[2])
    end
end
function tURK.vc_detachTowTruck(bl)
    local ay = globalVehicleOwnership[bl]
    if ay then
...
function tURK.vc_detachCargobob(bl)
    local ay = globalVehicleOwnership[bl]
    if ay then
...
function tURK.vc_toggleEngine(bl)
    local ay = globalVehicleOwnership[bl]
    if ay then
...
function tURK.ensureEntityOwnership(E)
    local al = 0
...
function tURK.vc_toggleLock(bl)
    local ay = globalVehicleOwnership[bl]
    if ay then
        local am = ay[2]
        local bI = GetVehicleDoorLockStatus(am) >= 2
        if tURK.ensureEntityOwnership(am) then
            if bI then
                ...
                tURK.notify("Vehicle locked.")
            end
        else
            tURK.notify("~r~Failed to get ownership of vehicle to lock/unlock.")
        end
    end
end

AddEventHandler("URK:johnnyCantMakeIt",function()
...
RegisterNetEvent("URK:viewRemoteDashcam",function(a9, bV)
    if cs ~= 0 then
        DestroyCam(cs, false)
        return
    end
...
local cx = 0
RegisterNetEvent("URK:displayVehicleBlip",function(a9)
    if cx ~= 0 then
        RemoveBlip(cx)
    end
    if a9 then
...
Citizen.CreateThread(function()
    ...
    while true do
        local aA, cy = tURK.getPlayerVehicle()
        if aA ~= 0 and cy then
            local cz = DecorGetBool(aA, "biometricLock")
            if cz then
                local y = tURK.getUserId()
                local biometricUsers = Entity(aA).state.biometricUsers
                local cA = DecorGetInt(aA, "vRP_owner")
                if y ~= cA and not tURK.isDev() and (not biometricUsers or not table.has(biometricUsers, y)) then
                    ...
                    SetVehicleMaxSpeed(tURK.getPlayerVehicle(), 0)
...
function setVehicleIdBiometricLock(bj, cB, cD)
    ...
    if cD and #cD > 0 then
        ...
        Citizen.CreateThread(function()
            ...
            TriggerServerEvent("URK:setBiometricUsersState", bV, cD)
        end)
    end
end
...
Citizen.CreateThread(
    function()
        while true do
            local c1, cu = tURK.getPlayerVehicle()
            if
                c1 ~= 0 and not globalHideUi and GetVehicleClass(c1) ~= 14 and
                    (cu or GetPedInVehicleSeat(c1, 0) == PlayerPedId())
             then
                SendNUIMessage({showSpeed = true, speed = math.ceil(GetEntitySpeed(c1) * 2.2369)})
            else
