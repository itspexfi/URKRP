RMenu.Add('URKclothing','mainMenu',RageUI.CreateMenu("","~r~Clothing Store",tURK.getRageUIMenuWidth(),tURK.getRageUIMenuHeight(),"banners","urk_clothesui"))
RMenu.Add('URKclothing','changePed',RageUI.CreateSubMenu(RMenu:Get('URKclothing','mainMenu'),"","~r~Change Gender"))
RMenu.Add('URKclothing','changeClothing',RageUI.CreateSubMenu(RMenu:Get('URKclothing','mainMenu'),"","~r~Change your Clothes"))
RMenu.Add('URKclothing','clearProps',RageUI.CreateSubMenu(RMenu:Get('URKclothing','mainMenu'),"","~r~Clear Props"))
...
Citizen.CreateThread(function()
    while true do 
        local G=GetEntityCoords(tURK.getPlayerPed())
        for H=1,#r do 
            if#(G-r[H])<=2.5 and not t then 
                ...
                h=true
                RageUI.Visible(RMenu:Get('URKclothing','mainMenu'),true)
            elseif s==H then 
                ...
                if not RageUI.Visible(RMenu:Get('URKclothing','changeClothing'))then 
                    F()
                end 
            end 
        end
    ...
    if not n then
        if RageUI.Visible(RMenu:Get('URKclothing', 'mainMenu')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                ...
                RageUI.Button('Apply Outfit Code','Applys an outfit code given by a player with ~y~URK Plus~w~ or ~y~URK Platinum~w~.',{RightLabel=">>>"},true,function(L,M,N)
                    if N then 
                        tURK.clientPrompt("Outfit Code:","",function(Q)
                            local R = string.gsub(Q, "%s+", "")
                            if #R == 5 then
                                TriggerServerEvent("URK:applyOutfitCode", string.upper(R))
                            else
                ...
                if PlayerIsInGang then
                    RageUI.Button('Apply Gang Outfit','Applys the outfit set by the Gang Leader.',{RightLabel=">>>"},true,function(L,M,N)
                            if N then
                                TriggerServerEvent("URK:applyGangFit", gangID)
                            end
                        end
                    )
                end
                RageUI.Button('Remove Props',nil,{RightLabel=">>>"},true,function(L,M,N)
                end,RMenu:Get('URKclothing','clearProps'))
            end)
        end
        if RageUI.Visible(RMenu:Get('URKclothing', 'changePed')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                RageUI.Button('MP Male','WARNING: Resets all your Body Options',{},true,function(L,M,N)
                    if N then 
                        if GetEntityHealth(PlayerPedId()) > 102 then
                            tURK.loadCustomisationPreset("DefaultMale")
                        else
                            tURK.notify("~r~Cannot change gender, you're dead.")
                        end
                    end 
                end,function()
                end)
                RageUI.Button('MP Female','WARNING: Resets all your Body Options',{},true,function(L,M,N)
                    if N then 
                        if GetEntityHealth(PlayerPedId()) > 102 then
                            tURK.loadCustomisationPreset("DefaultFemale")
                        else
                            tURK.notify("~r~Cannot change gender, you're dead.")
                        end
                    end 
        ...
        if RageUI.Visible(RMenu:Get('URKclothing', 'changeClothing')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                m=true
                if h then 
                    for H=1,#a,1 do 
                        ...
                        RageUI.List(P.name,get_drawables(P.type,P.index),a[H].currentListIndex,"Current Texture Index: "..k.textureN.."/"..j-1,{},true,function(L,M,N,Q)
                            ...
                            if N then 
                                ...
                                if k.index==2 then 
                                    ...
                                    TriggerServerEvent("URK:saveClothingHairData",GetPedDrawableVariation(tURK.getPlayerPed(),2),GetPedHairColor(tURK.getPlayerPed()))
...
function current_clothing(R,i,S)
    if R=="prop"then 
        if S then 
            if GetPedPropTextureIndex(tURK.getPlayerPed(),i)~=nil then 
                return GetPedPropTextureIndex(tURK.getPlayerPed(),i)
            else 
                return 0 
            end 
        else 
            if GetPedPropIndex(tURK.getPlayerPed(),i)~=nil then 
                if GetPedPropIndex(tURK.getPlayerPed(),i)==-1 then 
                    return GetPedPropIndex(tURK.getPlayerPed(),i)+2 
                else 
                    return GetPedPropIndex(tURK.getPlayerPed(),i)+1 
                end 
            else 
                return 0 
            end 
    ...
    elseif R=="drawable"then 
        ...
        elseif S then 
            if GetPedTextureVariation(tURK.getPlayerPed(),i)~=nil then 
                return GetPedTextureVariation(tURK.getPlayerPed(),i)
            else 
                return 0 
            end 
        else 
            if GetPedDrawableVariation(tURK.getPlayerPed(),i)~=nil then 
                return GetPedDrawableVariation(tURK.getPlayerPed(),i)+1 
            else 
                return 0 
            end 
        end 
...
function get_textures(T,R,i)
    if T=="drawable"then 
        ...
        j=GetNumberOfPedTextureVariations(tURK.getPlayerPed(),R,i-1)
        return j 
    else 
        j=GetNumberOfPedPropTextureVariations(tURK.getPlayerPed(),R,i-1)
        return j 
    end 
end
local function Y()
...
Citizen.CreateThread(function()
    while true do 
        if k.index~=nil and RageUI.Visible(RMenu:Get('URKclothing','changeClothing'))then 
...
AddEventHandler("URK:onClientSpawn",function(D, E)
    if E then
		for a7,a8 in pairs(r)do 
            tURK.addBlip(a8.x,a8.y,a8.z,73,3,"Clothing Store",0.7)
            tURK.addMarker(a8.x,a8.y,a8.z,0.6,0.6,0.6,10,255,81,170,50,9,false,false,true,"dp_clothing","top",90.0,90.0,0.0)
        end 
	end
end)
