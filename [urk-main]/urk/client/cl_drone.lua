...
local function d()
    ...
    if c.cameraEnabled then
        ...
        local f = 500 * math.ceil(#(tURK.getPlayerCoords() - GetEntityCoords(c.vehicleHandle)) / 10)
        ...
        local g = tURK.loadModel("prop_cs_tablet")
        c.tablet = CreateObject(g, tURK.getPlayerCoords(), true)
        ...
        tURK.loadAnimDict("amb@code_human_in_bus_passenger_idles@female@tablet@idle_a")
...
RegisterNetEvent("toggleDrone",function()
    if not c.active then
        if tURK.getPlayerVehicle() == 0 and not tURK.isInComa() then
            TriggerServerEvent("URK:performDroneLog", "Powered on their DJI Inspire Drone")
            newDrone()
        else
            tURK.notify("You are not able to use a drone right now.")
        end
    else
        TriggerServerEvent("URK:performDroneLog", "Powered off their DJI Inspire Drone")
        deleteDrone()
    end
end)
Citizen.CreateThread(function()
    ...
    while true do
        if c.active then
            ...
            if not NetworkHasControlOfEntity(c.driverHandle) or not NetworkHasControlOfEntity(c.vehicleHandle) then
                tURK.notify("You have ~b~lost control ~w~of the drone, after going out of range")
                deleteDrone()
            end
...
Citizen.CreateThread(function()
    while true do
        local p = tURK.getPlayerVehicle()
        if p ~= 0 then
            local q = GetEntityModel(p)
            if q == GetHashKey(a) then
                o = o + 1
                if o > 3 then
                    tURK.notify("~r~Access Denied~w~: Engaging defences in 3, 2, 1...")
                    Wait(500)
                    local r = tURK.getPlayerCoords()
                    AddOwnedExplosion(tURK.getPlayerPed(), r.x, r.y, r.z, 4, 17.0, true, false, 10.0)
                    ...
                    if tURK.getPlayerVehicle() ~= nil and DoesEntityExist(tURK.getPlayerVehicle()) then
                        DeleteEntity(tURK.getPlayerVehicle())
                    end
                end
            end
        end
...
function newDrone()
    local g = tURK.loadModel(a)
    local v = tURK.loadModel("s_m_m_bouncer_01")
    local e = tURK.getPlayerPed()
    local r = GetOffsetFromEntityInWorldCoords(e, 0.0, 4.0, 0.0)
    local w = GetEntityHeading(e)
    c.vehicleHandle = tURK.spawnVehicle(a, r, w, true)
    SetModelAsNoLongerNeeded(g)
    SetEntityNoCollisionEntity(tURK.getPlayerPed(), c.vehicleHandle, 1)
    Wait(1000)
    ...
    c.active = true
    tURK.notify("Drone ~b~created~w~.")
    Wait(1000)
    if globalNHSOnDuty then
        tURK.notify("~y~Reminder~w~: These drones are reserved for usage by HEMS, Specialised Off Road Unit")
        tURK.notify("and Senior Pilots only. If you are not in these divisions, you ~r~should not ~r~be using this.")
    end
    tURK.notify("To remove your drone, use ~b~/drone")
end
...
local function B()
    ...
    if p == 0 then
        if z then
            if #(z - tURK.getPlayerCoords()) > 20.0 then
                local C = PlayerPedId()
    ...
    if #(z - D) > 20.0 then
        ...
        if IsDisabledControlJustPressed(0, 23) or IsDisabledControlJustPressed(0, 75) then
            tURK.notify("You must be near the ~y~start position~w~ to exit.")
        end
    end
end
tURK.createThreadOnTick(B)
