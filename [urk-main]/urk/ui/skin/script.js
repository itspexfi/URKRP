$(document).ready(function(){
  ...
  $('input').change(function(){
    ...
    $.post('https://urk/updateSkin', JSON.stringify({
      value: false,
      // Face
  ...
  $(document).keypress(function(e) {
    ...
    if(e.which == 113){ // A pressed
      $.post('https://urk/rotaterightheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 101){ // E pressed
      $.post('https://urk/rotateleftheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 119 ){ // W pressed
      $.post('https://urk/zoomin', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 115){ // S pressed
      $.post('https://urk/zoomout', JSON.stringify({
        value: 10
      }));
    }
  });
  ...
  $('#skin-tabs skin-label').on('click', function(e){
    ...
    $.post('https://urk/zoom', JSON.stringify({
...
function leftWristComponent() {
    $.post('https://urk/updateLeftWristComponent', JSON.stringify({
...
function leftWristTexture() {
    ...
    $.post('https://urk/updateLeftWristTexture', JSON.stringify({
        componentID: $('.montre .active').attr('data'),
...
function rightWristComponent() {
    $.post('https://urk/updateRightWristComponent', JSON.stringify({
...
function rightWristTexture() {
    ...
    $.post('https://urk/updateRightWristTexture', JSON.stringify({
        componentID: $('.montre-right .active').attr('data'),
