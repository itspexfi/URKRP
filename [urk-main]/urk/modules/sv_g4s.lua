...
RegisterNetEvent("URK:toggleShiftG4S")
AddEventHandler("URK:toggleShiftG4S", function(Status)
    local source=source
    local user_id=URK.getUserId(source)
    if URK.hasGroup(user_id,"G4S Driver")then
        if Status then
            currentUsersOnG4SJob[user_id]={currentJob=""}
            TriggerClientEvent("URK:startShiftG4S",source,math.random(1000,9999))
            jobsCompleted[user_id] = 0
            Wait(10000)
            TriggerClientEvent("URK:updateJobInformation",source,GetCollection(user_id))
        else
            if currentUsersOnG4SJob[user_id]~=nil then
                currentUsersOnG4SJob[user_id]=nil
                jobsCompleted[user_id] = nil
                TriggerClientEvent("URK:endShiftG4S",source)
            end
        end
    else
        URKclient.notify(source,{"~r~You are not clocked on as currentUsersOnG4SJob G4S Driver."})
    end
end)

RegisterNetEvent("URK:acceptJob")
AddEventHandler("URK:acceptJob",function()
    local source=source
    local user_id=URK.getUserId(source)
    if URK.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
            if k == user_id then    
                ...
                TriggerClientEvent("URK:updateJobInformation",source,v.currentJob)
            end
        end
    else
        URKclient.notify(source,{"~r~You are not clocked on as currentUsersOnG4SJob G4S Driver."})
...
RegisterNetEvent("URK:updateMoneyInVehicle")
AddEventHandler("URK:updateMoneyInVehicle", function()
    local source=source
    local user_id=URK.getUserId(source)
    if URK.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
...
RegisterNetEvent("URK:updateMoneyOutVehicle")
AddEventHandler("URK:updateMoneyOutVehicle", function()
    local source=source
    local user_id=URK.getUserId(source)
    if URK.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
            ...
            if v.currentJob.jobActive then
                if URK.getUserSource(k)then
                    if not v.currentJob.collected then
                        if #(GetEntityCoords(GetPlayerPed(URK.getUserSource(k)))-v.currentJob.collectionCoords)<1.5 then
                            v.currentJob.collected=true
                            currentUsersOnG4SJob[k].currentJob=v.currentJob
                            TriggerClientEvent("URK:updateJobInformation",URK.getUserSource(k),v.currentJob)
                            TriggerClientEvent("URK:requestMoneyInVehicle",URK.getUserSource(k))
                        end
                    elseif v.currentJob.moneyInVehicle and not v.currentJob.depositing and not v.currentJob.moneyOutVehicle then
                        if #(GetEntityCoords(GetPlayerPed(URK.getUserSource(k)))-v.currentJob.depositCoords)<20 then
                            v.currentJob.depositing=true
                            currentUsersOnG4SJob[k].currentJob=v.currentJob
                            TriggerClientEvent("URK:updateJobInformation",URK.getUserSource(k),v.currentJob)
                            TriggerClientEvent("URK:requestMoneyOutVehicle",URK.getUserSource(k))
                        end
                    elseif not v.currentJob.deposited and v.currentJob.moneyOutVehicle then
                        if #(GetEntityCoords(GetPlayerPed(URK.getUserSource(k)))-v.currentJob.depositCoords)<1.5 then
                            v.currentJob.deposited=true
                            v.currentJob.depositing=false
                            TriggerClientEvent("URK:updateJobInformation",URK.getUserSource(k),GetCollection(k))
...
RegisterCommand('g4s', function(source, args)
    ...
    for k,v in pairs(currentUsersOnG4SJob) do
        if k == user_id then
            TriggerClientEvent("URK:openG4SMenu", source)
        end
    end
end)