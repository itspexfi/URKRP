...
AddEventHandler("URK:playerSpawn", function(user_id, source, first_spawn)
    if first_spawn then
        TriggerClientEvent('URK:policeSyncAllDoors', source, doors, doorStates)
    end
end)

RegisterNetEvent("URK:policeSetDoorState")
AddEventHandler("URK:policeSetDoorState", function(doorHash, state)
    local source = source
    local user_id = URK.getUserId(source)
    if URK.hasPermission(user_id, 'police.onduty.permission') then
        doorStates[doorHash] = state
        TriggerClientEvent('URK:policeSyncDoor', -1, doorHash, state)
    end
end)

RegisterNetEvent("URK:enterPoliceSyncDoors")
AddEventHandler("URK:enterPoliceSyncDoors", function()
    local source = source
    TriggerClientEvent('URK:gotAllDoorStates', source, doorStates)
end)