...
RegisterNetEvent('URK:getStoreRobBlips')
AddEventHandler('URK:getStoreRobBlips', function()
    local source = source
    TriggerClientEvent('URK:updateStoreRobBlips', -1, stores)
end)


RegisterNetEvent('URK:initiateStoreRobbery')
AddEventHandler('URK:initiateStoreRobbery', function(store)
    local source = source
    local user_id = URK.getUserId(source)
    if URK.hasPermission(user_id, "police.onduty.permission") then
        TriggerClientEvent('URK:resetStorePed', source, store)
    else
        if #URK.getUsersByPermission('police.onduty.permission') > 2 then
            for k,v in pairs(stores) do
                if k == store then
                    if v.cooldown == 0 then
                        ...
                        v.cooldown = 300
                        TriggerClientEvent('URK:updateStoreRobBlips', -1, stores)
                        TriggerClientEvent('URK:beginStoreRobbingAnimations', -1, store)
                        TriggerClientEvent('URK:storeRobberyInProgress', source, true, store)
                        ...
                        TriggerEvent('URK:PDRobberyCall', source, v.storename, v.position)
                    else
            ...
            URKclient.notify(source, {'~r~There are not enough police on duty to rob a store.'})
            TriggerClientEvent('URK:resetStorePed', source, store)
        end
    end
end)

RegisterNetEvent('URK:completeSafeCracking')
AddEventHandler('URK:completeSafeCracking', function(store)
    local source = source
    local user_id = URK.getUserId(source)
    URK.giveMoney(user_id, math.random(85000, 150000))
    TriggerClientEvent('URK:syncCloseSafeDoor', -1, store)
    TriggerClientEvent('URK:resetStorePed', -1, store)
    for k,v in pairs(stores) do
        if k == store then
            v.beingrobbed = false
        end
    ...
    TriggerClientEvent('URK:updateStoreRobBlips', -1, stores)
    TriggerClientEvent('URK:storeRobberyInProgress', source, false, store)
end)

RegisterNetEvent('URK:forceEndRobbery')
AddEventHandler('URK:forceEndRobbery', function(store)
    local source = source
    local user_id = URK.getUserId(source)
    TriggerClientEvent('URK:resetStorePed', -1, store)
    for k,v in pairs(stores) do
...
RegisterNetEvent('URK:syncOpenSafeDoor')
AddEventHandler('URK:syncOpenSafeDoor', function(store)
    local source = source
    TriggerClientEvent('URK:syncOpenSafeDoor', -1, store)
end)
