...
RegisterNetEvent("URK:attemptBeginDeliverooJob")
AddEventHandler("URK:attemptBeginDeliverooJob",function()
    local source=source
    local user_id=URK.getUserId(source)
    local deliverooJobTable={}
    if URK.hasGroup(user_id,"Deliveroo")then
        if not usersInDeliverooJob[user_id] then
            ...
            TriggerClientEvent("URK:beginDeliverooJob",source)
            Wait(100)
            TriggerClientEvent("URK:deliverooJobSetNextBlip",source,deliverooStops[deliverooJobTable.stopNumber].stopPosition)
        else
            URKclient.notify(source,{"~r~You are already a Deliveroo Driver."})
        end
    else
        URKclient.notify(source,{"~r~You are not clocked on as a Deliveroo Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(deliverooStops)do
                    if URK.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(URK.getUserSource(k)))-deliverooStops[v.currentJob.stopNumber].stopPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            URK.giveBankMoney(k,pay)
                            TriggerClientEvent("URK:deliverooJobReachedNextStop",URK.getUserSource(k),pay)
                            if v.currentJob.stopNumber==#deliverooStops then
                                TriggerClientEvent("URK:deliverooJobEnd", URK.getUserSource(k), true)
                                v.currentJob.jobActive=false
                                usersInDeliverooJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("URK:deliverooJobSetNextBlip",URK.getUserSource(k),deliverooStops[v.currentJob.stopNumber].stopPosition)
                            end
                        end
                    end
                end
