...
RegisterNetEvent("URK:attemptBeginBusJob")
AddEventHandler("URK:attemptBeginBusJob",function()
    local source=source
    local user_id=URK.getUserId(source)
    local BusJobTable={}
    if URK.hasGroup(user_id,"Bus Driver")then
        if not usersInBusJob[user_id] then
            ...
            TriggerClientEvent("URK:beginBusJob",source)
            Wait(100)
            TriggerClientEvent("URK:setNextBusJobBlip",source,busJobStops[BusJobTable.stopNumber].stopPosition)
        else
            URKclient.notify(source,{"~r~You are already a Bus Driver."})
        end
    else
        URKclient.notify(source,{"~r~You are not clocked on as a Bus Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(busJobStops)do
                    if URK.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(URK.getUserSource(k)))-busJobStops[v.currentJob.stopNumber].stopPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            URK.giveBankMoney(k,pay)
                            TriggerClientEvent("URK:nextStopReachedBusJob",URK.getUserSource(k),pay)
                            if v.currentJob.stopNumber==#busJobStops then
                                TriggerClientEvent("URK:endBusJob", URK.getUserSource(k))
                                v.currentJob.jobActive=false
                                usersInBusJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("URK:setNextBusJobBlip",URK.getUserSource(k),busJobStops[v.currentJob.stopNumber].stopPosition)
                            end
                        end
                    end
                end
