...
function URK.getUserGroups(user_id)
...
function URK.addUserGroup(user_id, group)
    if not URK.hasGroup(user_id, group) then
        ...
        if ngroup then
            ...
            TriggerEvent("URK:playerJoinGroup", user_id, group, gtype)
        end
    end
end
...
function URK.getUserGroupByType(user_id, gtype)
    local user_groups = URK.getUserGroups(user_id)
    for k, v in pairs(user_groups) do
        local kgroup = groups[k]
        if kgroup then
...
function URK.getUsersByGroup(group)
    local users = {}

    for k, v in pairs(URK.rusers) do
        if URK.hasGroup(tonumber(k), group) then
...
function URK.getUsersByPermission(perm)
    local users = {}

    for k, v in pairs(URK.rusers) do
        if URK.hasPermission(tonumber(k), perm) then
            table.insert(users, tonumber(k))
        end
    end
...
function URK.removeUserGroup(user_id, group)
    local user_groups = URK.getUserGroups(user_id)
    ...
    TriggerEvent("URK:playerLeaveGroup", user_id, group, gtype)

    user_groups[group] = nil -- remove reference
end

-- check if the user has a specific group
function URK.hasGroup(user_id, group)
    local user_groups = URK.getUserGroups(user_id)
...
-- check if the user has a specific permission
function URK.hasPermission(user_id, perm)
    local user_groups = URK.getUserGroups(user_id)
    ...
    if fchar == "@" then -- special aptitude permission
        ...
        if #parts == 3 then -- decompose group.aptitude.operator
            ...
            local aurk = math.floor(URK.expToLevel(URK.getExp(user_id, group, aptitude)))

            local fop = string.sub(op, 1, 1)
            if fop == "<" then -- less (group.aptitude.<x)
                local urk = parseInt(string.sub(op, 2, string.len(op)))
                if aurk < urk then
                    return true
                end
            elseif fop == ">" then -- greater (group.aptitude.>x)
                ...
                local urk = parseInt(string.sub(op, 1, string.len(op)))
                if aurk == urk then
                    return true
                end
            end
    ...
    elseif fchar == "#" then -- special item permission
        ...
        if #parts == 2 then -- decompose item.operator
            ...
            local amount = URK.getInventoryItemAmount(user_id, item)
...
function URK.hasPermissions(user_id, perms)
