function tURK.add3DTextForCoord(a, b, c, d, e)
    local function f(g)
        tURK.DrawText3D(g.coords, g.text, 0.2)
    end
    local h = tURK.generateUUID("3dtext", 8, "alphanumeric")
    tURK.createArea("3dtext_" .. h,vector3(b, c, d),e,6.0,function()end,function()end,f,{coords = vector3(b, c, d), text = a})
end
function tURK.drawFloatingHelpText(a, i)
...
function tURK.DrawText3D(i, a, j, k, l, m, noShadow, noOutline)
...
function tURK.DrawText(b, c, a, j, k, p, m, q)
    SetTextScale(j, j)
    SetTextFont(k or 0)
    if p then
...
function tURK.DrawSprite3d(data)
